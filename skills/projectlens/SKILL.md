...
Two artefacts in `projectlens-out/`:
...
Plus `lens.json`, `lens.sections.json`, and `manifest.json` inside `projectlens-out/`.
...
```bash
python ${CLAUDE_PLUGIN_ROOT}/skills/projectlens/scripts/scan.py <target-path> [flags]
```
...
If the user follows up with "use this as my context," append the capsule to their `CLAUDE.md` wrapped in `<!-- projectlens-begin -->` / `<!-- projectlens-end -->` markers.

## Sub-commands

| Command | What it does | Reference |
|---|---|---|
| `/projectlens compact` | Generates WORKING_CONTEXT.md from session state — lets user `/clear` with continuity | `references/conversation-compactor.md` |
| `/projectlens stats` | Shows lifetime savings report (run `scripts/stats_cli.py`) | `references/telemetry.md` |
| `/projectlens diff <a..b>` | Structural diff between two git refs | `references/complexity-tiers.md` |
...
- Do **not** rebuild a lens when `projectlens-out/manifest.json` shows current hashes. Link to the existing one.
...
- `references/conversation-compactor.md` — Phase 4 `/projectlens compact`
