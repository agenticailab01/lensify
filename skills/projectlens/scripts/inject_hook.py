...
When the user submits a prompt, this hook:
    1. Loads projectlens-out/lens.sections.json (if present)
...
def _load_sections(project_root: str) -> dict | None:
    """Load lens.sections.json from projectlens-out/ if present."""
    p = Path(project_root) / "projectlens-out" / "lens.sections.json"
    if not p.exists():
        return None
    try:
...
def _load_session_capsule(project_root: str) -> str:
    p = Path(project_root) / "projectlens-out" / "SESSION.capsule.md"
    if not p.exists():
        return ""
    try:
...
def handle(payload: dict) -> None:
    ...
    parts: list[str] = []
    header = "[ProjectLens] Relevant context for your question:"
    parts.append(header)
    for name in chosen:
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        sys.stderr.write(f"[projectlens-inject] hook error: {exc}\n")
        _emit({})
    return 0
