...
Two modes:
    ...
    - LLM-enhanced (--llm or env PROJECTLENS_COMPACT_LLM=1): asks Claude
...
Output: writes projectlens-out/WORKING_CONTEXT.md and prints a banner JSON
...
def build_working_context(state: SessionState, *, use_llm: bool = False) -> tuple[str, dict]:
    ...
    sections = [
        f"# Working Context — projectlens v0.9.0\n\n"
...
def run_compact(
    ...
    *,
    ...
    out_dir = Path(output_dir) if output_dir else project_root / "projectlens-out"
    ...
    if not memory_disabled():
        try:
            ...
            sys.stderr.write(f"[projectlens-memory] save failed: {exc}\n")
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(description="ProjectLens conversation compactor.")
    ...
    use_llm = args.llm or os.environ.get("PROJECTLENS_COMPACT_LLM") in ("1", "true", "yes", "on")
    try:
