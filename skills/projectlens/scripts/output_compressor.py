...
OUTPUT_CACHE_DIRNAME = ".projectlens-outputs"
...
def _store_raw(text: str, project_root: Path) -> str:
    """Write the raw output to .projectlens-outputs/<sha256-12>.txt."""
...
def format_for_agent(result: CompressionResult) -> str:
    ...
    lines = [
        f"[ProjectLens] Tool output ({result.original_bytes:,} bytes, "
...
def is_disabled() -> bool:
    """Opt out via PROJECTLENS_COMPRESS_OUTPUT=0."""
    val = os.environ.get("PROJECTLENS_COMPRESS_OUTPUT")
    if val is None:
        # Falls back to the global PROJECTLENS_DEDUP switch
        val = os.environ.get("PROJECTLENS_DEDUP")
