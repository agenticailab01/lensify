...
MEMORY_DIRNAME = ".projectlens-memory"
...
def is_disabled() -> bool:
    """Opt out via PROJECTLENS_MEMORY=0."""
    val = os.environ.get("PROJECTLENS_MEMORY")
...
def _atomic_write_json(path: Path, data: dict | list) -> None:
    ...
    try:
        ...
        import sys
        sys.stderr.write(f"[projectlens-memory] write failed: {exc}\n")
...
def format_memories_for_injection(memories: list[MemoryEntry]) -> str:
    ...
    if not memories:
        return ""
    lines = ["[ProjectLens] Memories from previous sessions in this project:"]
