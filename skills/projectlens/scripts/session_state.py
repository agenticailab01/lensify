...
keyed by absolute path + content hash. Persists to .projectlens-session.json
in the project root.
...
STATE_FILENAME = ".projectlens-session.json"
...
def save_state(state: SessionState, project_root: str | Path) -> None:
    ...
    try:
        ...
        with tempfile.NamedTemporaryFile(
            ...
            dir=str(path.parent), prefix=".projectlens-session-", suffix=".tmp",
        ) as tmp:
...
def _log_warn(msg: str) -> None:
    ...
    try:
        import sys
        print(f"[projectlens-dedup] {msg}", file=sys.stderr)
    except Exception:
        pass


def is_disabled() -> bool:
    """Allow users to opt out via PROJECTLENS_DEDUP=0."""
    return os.environ.get("PROJECTLENS_DEDUP", "1") in ("0", "false", "no", "off")
