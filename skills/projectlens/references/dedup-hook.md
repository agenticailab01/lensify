...
ProjectLens ships a `PreToolUse` hook that intercepts every `Read` tool call
...
1. **`SessionStart`** hook fires when Claude opens a new session — clears
   `.projectlens-session.json` and starts a fresh tracker.
...
3. State persists in `.projectlens-session.json` in the project root.
...
```bash
export PROJECTLENS_DEDUP=0   # disables both hooks
```

## Inspect session state

```bash
python /path/to/projectlens/skills/projectlens/scripts/dedup_hook.py --stats /path/to/project
```
