...
The statusline badge and `/projectlens stats` command surface the cumulative
...
```
~/.projectlens/stats.json
```

Override via `PROJECTLENS_STATS_HOME` (used by tests + power users who want a
...
`PROJECTLENS_USD_PER_MTOK` for a different model:

```bash
export PROJECTLENS_USD_PER_MTOK=0.80   # Haiku rate
projectlens stats                       # numbers now reflect Haiku pricing
```
...
- Stats live in `~/.projectlens/stats.json`. Wipe at any time with
  `--reset` or `rm ~/.projectlens/stats.json`.
...
```bash
export PROJECTLENS_STATS=0   # disable lifetime tracking
```
