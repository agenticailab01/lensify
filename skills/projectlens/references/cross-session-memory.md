...
ProjectLens v0.5.0 adds project-local persistent memory: each time the user
runs `/projectlens compact`, the WORKING_CONTEXT.md is *also* saved as a
...
Claude-Mem uses Chroma embeddings (95.2% recall on LongMemEval); ProjectLens
...
```
<project-root>/.projectlens-memory/
...
```
[ProjectLens] Memories from previous sessions in this project:
...
`/projectlens compact` or by the skill flow). The compactor:
...
If the user never runs `/projectlens compact`, no memories accumulate. This
...
- Opt out: `PROJECTLENS_MEMORY=0`.
- Wipe: `rm -rf .projectlens-memory/`.
- Recommended `.gitignore` entry: `.projectlens-memory/` (default-private).
  Teams that *want* shared memory can commit it intentionally.

## Trade-offs vs Claude-Mem

| Property | ProjectLens memory | Claude-Mem |
|---|---|---|
...
developers), ProjectLens memory is enough. For users with cross-project,
high-recall needs (the common case for full-time AI engineers), Claude-Mem
remains better — and ProjectLens deliberately doesn't compete on that axis.

The two stack: if you have Claude-Mem installed, ProjectLens memory still
