...
Lensify v0.5.0 ships a PostToolUse hook that detects large textual tool
...
```
[Lensify] Tool output (28,341 bytes, detected as `pytest`) —
...
Full raw output saved to `.lensify-outputs/3f8a2c1b9d04.txt`.
...
For users who want MCP-level interception, Lensify stacks cleanly with
Context Mode — install both, and Context Mode handles the immediate-turn
compression while Lensify handles project orientation + session memory.
...
```
<project-root>/.lensify-outputs/<sha256-12>.txt
```
...
Recommended `.gitignore` addition: `.lensify-outputs/`.

## Opt out

```bash
export LENSIFY_COMPRESS_OUTPUT=0    # disable just compression
export LENSIFY_DEDUP=0              # disable ALL lensify hooks
```
