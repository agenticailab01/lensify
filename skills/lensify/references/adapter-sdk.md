...
A complete contributor guide for adding framework support to Lensify. Read this top to bottom before writing your first adapter.
...
Built-in adapters live in `skills/lensify/scripts/frameworks/<pack>/`. User-defined adapters drop into `<project>/.lensify/frameworks/`.
...
Add an entry to `skills/lensify/scripts/frameworks/manifest.json`:
...
Drop adapter modules in `<your-project>/.lensify/frameworks/`. They're auto-discovered per-scan. The loader:
...
**Tests fail with "perf budget exceeded".** Your `extract()` is too slow or reads too many files. Profile with `python3 -m cProfile -o out.prof skills/lensify/scripts/scan.py /path/to/project`.
