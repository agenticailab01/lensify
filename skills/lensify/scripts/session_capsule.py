...
def build_session_capsule(state: SessionState) -> str:
    ...
    body_parts = ["<!-- lensify-session-begin -->"]
    for name, _ in _SECTION_ORDER:
        s = sections.get(name)
        if s:
            body_parts.append(s)
    body_parts.append("<!-- lensify-session-end -->")
    return "\n\n".join(body_parts) + "\n"


def write_session_capsule(state: SessionState, project_root: str | Path) -> Path:
    """Write SESSION.capsule.md to lensify-out/ in the project root."""
    out_dir = Path(project_root) / "lensify-out"
