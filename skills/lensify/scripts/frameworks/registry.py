...
USER_FRAMEWORKS_DIRNAME = ".lensify/frameworks"  # per-project drop-in
...
def discover_adapter_classes(
    ...
    *,
    ...
    Order:
        ...
        2. User-defined adapters in <project>/.lensify/frameworks/*.py
    """
...
def _load_user_adapters(user_dir: Path) -> list[type[FrameworkAdapter]]:
    """Load any *.py files in <project>/.lensify/frameworks/ as adapters.
    ...
    that repo execute code in the agent's environment. To avoid this:
      ...
      • Power users set LENSIFY_USER_ADAPTERS=1 in their shell rc to
    ...
    """
    import os
    if os.environ.get("LENSIFY_USER_ADAPTERS", "").strip() != "1":
        return []
