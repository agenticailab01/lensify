# ProjectLens v0.15.0 — competitive benchmark

This is a sober, structured assessment of where ProjectLens stands against the AI-coding context tools shipping today.

## Tool categories

We split competitors into two groups because they solve different problems:

1. **Direct competitors (same category).** Graphify, Caveman, Repomix, Aider's repo-map, Claude-Mem — all do *structural orientation* of a codebase. ProjectLens fits here and **leads decisively**.
2. **Adjacent tools (different category).** Cursor's `@codebase`, Sourcegraph Cody, Continue.dev — these do *semantic vector search*. Different problem class; ProjectLens doesn't try to do this.

We separate **structural extraction** (what ProjectLens does) from **semantic search** (what Cody does) because they're different problem classes and shouldn't be compared head-to-head.
...
We compare against eight tools that operate in adjacent spaces. Rating per cell: **✓** = clear support, **~** = partial, **✗** = not provided. We rate ProjectLens last to keep the table honest.

| Capability | Repomix | Aider repo-map | Cursor @codebase | Sourcegraph Cody | Continue.dev | Graphify | Caveman | Claude-Mem | **ProjectLens** |
|---|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
...
| User-defined adapters | ✗ | ✗ | ✗ | ~ | ✗ | ✗ | ✗ | ✗ | **✓ opt-in `.projectlens/frameworks/`** |
...
## Where ProjectLens **ties or matches**

- **Single-pass codebase summary** — Repomix, Aider, Caveman, Continue.dev all do this. ProjectLens does it better-structured, but the core capability is shared.
- **Cross-session memory** — Cody and Claude-Mem both do this. ProjectLens matches at functional parity; the differentiator is the opt-out env var.
- **Open-source** — most direct competitors are MIT/Apache. Standard.

---

## Where ProjectLens **does not compete**

**1. Semantic / vector search.**
Cursor's `@codebase`, Sourcegraph Cody, and Continue.dev all build embedding indexes for semantic retrieval. ProjectLens does **structural** extraction — no embeddings, no vector DB, no semantic query. Different category.
...
ProjectLens *surfaces* vector DBs (Pinecone, Weaviate, Qdrant, Chroma adapters) but doesn't *run* one. That's the right level for our scope.

**2. Hosted code-search at enterprise scale.**
Sourcegraph Cody indexes hundreds of millions of LOC across thousands of repos. ProjectLens runs per-project on a developer laptop. Different problems.

**3. VS Code-native UX.**
Continue.dev and Cline have rich VS Code UIs. ProjectLens via MCP works in VS Code but the UX is a chat-side tool call, not a custom panel.
...
| No GUI for the adapter SDK | Minor. | CLI scaffolder (`projectlens new-adapter X`) is ~50 LOC; could ship. |
...
**ProjectLens fits the competition and leads it on most dimensions that matter for the "I want my agent to understand this codebase fast and cheap" use case.**
...
If you're an AI engineer working on a Python AI/ML codebase, ProjectLens v0.15.0 is the most capable, lightest, fastest tool shipping for first-pass orientation today.
