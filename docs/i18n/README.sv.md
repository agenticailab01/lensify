...
15. [Utöka — skriv din egen adapter](#extending-lensify)
...
Lensify väljer rätt djup automatiskt. Överskrid med `--tier T1|T2|T3` endast när du har en stark anledning.
...
Överskridningstips i chatten — Lensify läser avsikt:
...
En enda Lensify-skanning på ett AI-utvecklingsprojekt blottlägger varje länk i kedjan: från råa notebooks genom träning, modellering, embeddings, agentisk orkestrering, UI-komponenter och produktionsdeployments — allt i **en kapsel**, inom budget.
...
**Cowork-begränsning:** endast SessionStart utlöses i Coworks hookyta. Skanningsmotorn, kapselgenerering och `/lensify compact` fungerar fortfarande — men de 5 hookdrivna optimeringarna aktiveras endast i Claude Codes terminal-CLI.
...
**Inget skickas off-device** om du inte uttryckligen kör `/lensify compact --llm`. Stats- och minnesfiler är ren JSON — granskningsbara, raderbara, ingen PII.
...
Släpp `.py`-filer i `<your-project>/.lensify/frameworks/`. De auto-upptäcks per skanning när du opt-in:
...
Lensify är **det lätta, deterministiska orienteringsskiktet** i AI-kodningsassistentutrymmet. Det kompletterar semantiska sökverktyg snarare än ersätter dem.
...
S: Nej, om du inte uttryckligen kör `/lensify compact --llm` OCH har `ANTHROPIC_API_KEY` inställd. Även då skickas endast sessionsaktivitetssammanfattningen (filsökvägar, kommandonamn, testresultat) — aldrig filinnehåll.
...
⭐ Stjärnmarkera detta repo om Lensify sparar dig tokens. Öppna ett issue om ett framework du använder inte täcks — de flesta adaptrar är ~100 LOC och vi prioriterar.
