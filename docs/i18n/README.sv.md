...
> 🌐 **Svenska** — Tillbaka till engelska: [English](../../README.md)
...
> **Adaptiv enkelskannings-projektlins + tokenoptimerad kontextkapsel för AI-kodningsagenter.** Minskar orienteringstoken med 70–90%. Frameworkmedveten över hela AI-utvecklingens livscykel. Ren standardbibliotek. MIT-licensierad.

**📖 Läs på ditt språk:** 🇬🇧 [← Tillbaka till engelska](../../README.md) · 🇨🇳 [简体中文](README.zh-CN.md) · 🇹🇼 [繁體中文](README.zh-TW.md) · 🇯🇵 [日本語](README.ja.md) · 🇰🇷 [한국어](README.ko.md) · 🇩🇪 [Deutsch](README.de.md) · 🇫🇷 [Français](README.fr.md) · 🇪🇸 [Español](README.es.md) · 🇮🇳 [हिन्दी](README.hi.md) · 🇧🇷 [Português](README.pt.md) · 🇷🇺 [Русский](README.ru.md) · 🇸🇦 [العربية](README.ar.md) · 🇮🇹 [Italiano](README.it.md) · 🇵🇱 [Polski](README.pl.md) · 🇳🇱 [Nederlands](README.nl.md) · 🇹🇷 [Türkçe](README.tr.md) · 🇺🇦 [Українська](README.uk.md) · 🇻🇳 [Tiếng Việt](README.vi.md) · 🇮🇩 [Bahasa Indonesia](README.id.md) · 🇬🇷 [Ελληνικά](README.el.md) · 🇷🇴 [Română](README.ro.md) · 🇨🇿 [Čeština](README.cs.md) · 🇫🇮 [Suomi](README.fi.md) · 🇩🇰 [Dansk](README.da.md) · 🇳🇴 [Norsk](README.no.md) · 🇭🇺 [Magyar](README.hu.md) · 🇹🇭 [ภาษาไทย](README.th.md) · 🇺🇿 [O'zbekcha](README.uz.md)

---

## Innehållsförteckning

1. [Varför ProjectLens](#why-projectlens)
2. [En överblick](#at-a-glance)
3. [Snabbstart](#quick-start)
4. [Hur det fungerar](#how-it-works)
5. [Installation per verktyg (4 kanaler)](#installation-by-tool)
6. [Adaptiva nivåer — T1 / T2 / T3](#adaptive-tiers)
7. [Frameworktäckning — 30 adaptrar i 8 pack](#framework-coverage)
8. [Sessionshooks — 5 produktionshooks](#session-hooks)
9. [Konversationskompaktor](#conversation-compactor)
10. [Token-ekonomi — konkreta siffror](#token-economics)
11. [Tester och prestandabudgetar](#tests--performance)
12. [Säkerhet och styrning](#security--governance)
13. [Konfigurations-miljövariabler](#configuration)
14. [Projektstruktur](#project-structure)
15. [Utöka — skriv din egen adapter](#extending-projectlens)
16. [Jämförelse med alternativ](#comparison)
17. [FAQ](#faq)
18. [Vägkarta](#roadmap)
19. [Bidra och licens](#contributing--license)

---

## Varför ProjectLens

Moderna AI-kodningsagenter har ett kontextfönsterproblem: ju större projektet, desto fler tokens bränner de bara på att **orientera sig**. Ett typiskt onboardingflöde läser 20–40 filer innan agenten kan göra användbart arbete — det är 10–30k tokens som spenderas på att förstå, inte att lösa användarens faktiska problem.

ProjectLens ersätter den orienteringsfasen med en **enda skanning** (under 100 ms) som producerar ett tokenbegränsat, framework-medvetet kontextblock. Agenten läser **en kapsel** istället för dussintals filer. Tokenanvändning minskar 70–90% bara från orienteringsbesparingar — och de 5 sessionshooksen staplar ytterligare besparingar ovanpå.

**Avvägningen den gör:** deterministisk strukturell extraktion (snabb, gratis, framework-medveten) istället för semantisk vektorsökning (långsammare, embeddingkostnad, generisk). ProjectLens fungerar tillsammans med semantiska verktyg som Cursor's `@codebase` och Sourcegraph Cody — inte mot dem. Använd ProjectLens för omedelbar orientering; använd semantisk sökning när agenten behöver hitta något specifikt genom betydelse snarare än struktur.
...
📖 Fullständiga steg-för-steg-instruktioner i **[`USER-INSTALL.md`](../../USER-INSTALL.md)**
---

## Hur det fungerar

Skanningsmotorn kör fem faser per anrop:
...
Typisk körtid: **30 ms** på medelstora projekt, **113 ms** på ett 500-filsprojekt. Skanningen läser aldrig om filer mellan faser.

**Två artefakter per skanning:**
...
## Installation per verktyg (4 kanaler)

Fyra distributionskanaler delar samma skanningsmotor. Välj den som matchar ditt verktyg.

### Channel 1 — Claude Code / Cowork plugin (recommended)

Den kompletta upplevelsen: alla 5 hooks utlöses, slash-kommandon, statusrad, memory loader.
...
Ren stdlib JSON-RPC 2.0 stdio-server. Ingen `pip install`-steg behövs för själva servern — den körs direkt från det klonade repot med endast Python-standardbiblioteket.
...
## Adaptiva nivåer — T1 / T2 / T3

ProjectLens väljer rätt djup automatiskt. Överskrid med `--tier T1|T2|T3` endast när du har en stark anledning.
...
Överskridningstips i chatten — ProjectLens läser avsikt:
...
## Frameworktäckning — 30 adaptrar i 8 pack

30 adaptrar över 8 pack. Varje adapter är ~80–120 LOC, lazy-laddad endast när dess frameworksignatur matchar projektet.
...
En enda ProjectLens-skanning på ett AI-utvecklingsprojekt blottlägger varje länk i kedjan: från råa notebooks genom träning, modellering, embeddings, agentisk orkestrering, UI-komponenter och produktionsdeployments — allt i **en kapsel**, inom budget.

---

## Sessionshooks — 5 produktionshooks

5 hooks samverkar över en Claude Code-session. Tokenbesparingar staplas över sessionens hela livslängd.
...
**Cowork-begränsning:** endast SessionStart utlöses i Coworks hookyta. Skanningsmotorn, kapselgenerering och `/projectlens compact` fungerar fortfarande — men de 5 hookdrivna optimeringarna aktiveras endast i Claude Codes terminal-CLI.

---

## Konversationskompaktor

Långa sessioner äter kontext. Kompaktorn återvinner 8–25k tokens på sekunder.
...
Kompaktorn läser sessionstillstånd inspelat av `activity_hook` och genererar en `WORKING_CONTEXT.md` som sammanfattar:
...
Tomtillståndsvarning: om inga PostToolUse hooks utlöstes (typiskt i Cowork), ger kompaktorn ut en tydlig diagnos istället för att låtsas ha fångat arbete.

---

## Token-ekonomi — konkreta siffror

Var besparingarna kommer ifrån — konkreta siffror från produktionsanvändning:
...
**Kostnadsexempel (Opus-prissättning, $15/Mtok input):** en 4-timmars kodningssession som tidigare kostade ~$3 i input-tokens sjunker till ~$0,45–$0,90 med alla hooks aktiva.

---

## Tester och prestandabudgetar

**527 enhetstester** + **17 CI-verkställda prestanda- och säkerhetsbudgetar** körs vid varje commit över macOS / Linux / Windows × Python 3.9–3.12.

Uppmätt prestanda på syntetiska projektstorlekar:
...
Kapselstorleken förblir **begränsad av tierbudget oavsett projektstorlek** — linjär skanningstid, konstant output. Det är den arkitektoniska vallgraven.

Hårda tak verkställda i CI:
...
Att lägga till adaptrar kan inte regressera någon av dessa. Pull Requests som bryter mot en budget får CI att misslyckas.
...
**Inget skickas off-device** om du inte uttryckligen kör `/projectlens compact --llm`. Stats- och minnesfiler är ren JSON — granskningsbara, raderbara, ingen PII.

For governance — what contributions we accept and what we don't — see [`GOVERNANCE.md`](../../GOVERNANCE.md).

---

## Konfigurations-miljövariabler

Alla persistenta ytor har dokumenterade env var opt-outs:
...
Avaktivera för att återaktivera. Inget annat förändras — inga pluginfiler flyttade, ingen data förlorad.
...
## Utöka — skriv din egen adapter

Adapter-SDK är avsiktligt liten. Varje adapter är ~80–120 LOC.
...
Kontraktet varje adapter måste följa:
...
Släpp `.py`-filer i `<your-project>/.projectlens/frameworks/`. De auto-upptäcks per skanning när du opt-in:

```bash
export PROJECTLENS_USER_ADAPTERS=1
```

Förtroendebeslutet sker i din skal, inte i koden som skannas — skyddar dig från skadliga adaptrar i opålitliga repos.

---

## Jämförelse med alternativ

ProjectLens är **det lätta, deterministiska orienteringsskiktet** i AI-kodningsassistentutrymmet. Det kompletterar semantiska sökverktyg snarare än ersätter dem.
...
S: Grundskanningen (filgång, språkdetektering, symboler) fungerar på JS, TS, Go, Java, Rust, Ruby, PHP, C/C++ och ~20 andra språk. De 30 frameworkspecifika adaptrarna är för närvarande mest Python-fokuserade, med Vue SFC + Tailwind + Docker Compose som täcker JS/web-sidan. JS/TS framework-adaptrar (Next.js, Astro, SvelteKit, NestJS) finns på färdplanen.

**Q: Will it slow down my Claude Code session?**
S: Hook subprocess-start är begränsad till 250 ms kallt (vanligtvis 20–30 ms varmt). Hook-output är begränsad till 500 tokens per händelse. Båda gränserna är CI-verkställda. Själva skanningen körs on-demand, inte per prompt.

**Q: Does it send my code anywhere?**
S: Nej, om du inte uttryckligen kör `/projectlens compact --llm` OCH har `ANTHROPIC_API_KEY` inställd. Även då skickas endast sessionsaktivitetssammanfattningen (filsökvägar, kommandonamn, testresultat) — aldrig filinnehåll.

**Q: Is the capsule different from a README?**
S: En README berättar för **människor** vad projektet gör. Kapseln berättar för din **agent** vad projektet innehåller: rutter, modeller, träningsloopar, vektorindex, deployments etc. Olika publik, olika output, båda har sin plats.
...
**Q: Is it free?**
S: Ja — MIT-licensierad. Ingen prenumeration, ingen off-device-telemetri, inga obligatoriska pip-beroenden.
...
Rösta med GitHub issues — vad ska komma först?
...
⭐ Stjärnmarkera detta repo om ProjectLens sparar dig tokens. Öppna ett issue om ett framework du använder inte täcks — de flesta adaptrar är ~100 LOC och vi prioriterar.
