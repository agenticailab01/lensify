...
> 🌐 **Deutsch** — Zurück zur englischen Version: [English](../../README.md)
...
> **Single-Scan adaptive Projektlinse + tokenoptimierte Kontextkapsel für KI-Coding-Agenten.** Reduziert Orientierungstokens um 70–90%. Framework-bewusst über den gesamten KI-Entwicklungslebenszyklus. Reine Standardbibliothek. MIT-lizenziert.

**📖 In deiner Sprache lesen:** 🇬🇧 [← Zurück zur englischen Version](../../README.md) · 🇨🇳 [简体中文](README.zh-CN.md) · 🇹🇼 [繁體中文](README.zh-TW.md) · 🇯🇵 [日本語](README.ja.md) · 🇰🇷 [한국어](README.ko.md) · 🇫🇷 [Français](README.fr.md) · 🇪🇸 [Español](README.es.md) · 🇮🇳 [हिन्दी](README.hi.md) · 🇧🇷 [Português](README.pt.md) · 🇷🇺 [Русский](README.ru.md) · 🇸🇦 [العربية](README.ar.md) · 🇮🇹 [Italiano](README.it.md) · 🇵🇱 [Polski](README.pl.md) · 🇳🇱 [Nederlands](README.nl.md) · 🇹🇷 [Türkçe](README.tr.md) · 🇺🇦 [Українська](README.uk.md) · 🇻🇳 [Tiếng Việt](README.vi.md) · 🇮🇩 [Bahasa Indonesia](README.id.md) · 🇸🇪 [Svenska](README.sv.md) · 🇬🇷 [Ελληνικά](README.el.md) · 🇷🇴 [Română](README.ro.md) · 🇨🇿 [Čeština](README.cs.md) · 🇫🇮 [Suomi](README.fi.md) · 🇩🇰 [Dansk](README.da.md) · 🇳🇴 [Norsk](README.no.md) · 🇭🇺 [Magyar](README.hu.md) · 🇹🇭 [ภาษาไทย](README.th.md) · 🇺🇿 [O'zbekcha](README.uz.md)

---

## Inhaltsverzeichnis

1. [Warum ProjectLens](#why-projectlens)
2. [Auf einen Blick](#at-a-glance)
3. [Schnellstart](#quick-start)
4. [Funktionsweise](#how-it-works)
5. [Installation nach Tool (4 Kanäle)](#installation-by-tool)
6. [Adaptive Stufen — T1 / T2 / T3](#adaptive-tiers)
7. [Framework-Abdeckung — 30 Adapter in 8 Paketen](#framework-coverage)
8. [Session-Hooks — 5 Produktions-Hooks](#session-hooks)
9. [Konversations-Kompaktor](#conversation-compactor)
10. [Token-Ökonomie — konkrete Zahlen](#token-economics)
11. [Tests & Leistungsbudgets](#tests--performance)
12. [Sicherheit & Governance](#security--governance)
13. [Konfigurations-Umgebungsvariablen](#configuration)
14. [Projektstruktur](#project-structure)
15. [Erweitern — eigenen Adapter schreiben](#extending-projectlens)
16. [Vergleich mit Alternativen](#comparison)
17. [FAQ](#faq)
18. [Roadmap](#roadmap)
19. [Mitwirken & Lizenz](#contributing--license)

---

## Warum ProjectLens

Moderne KI-Coding-Agenten haben ein Kontextfenster-Problem: Je größer das Projekt, desto mehr Tokens verbrennen sie nur, um sich **selbst zu orientieren**. Ein typischer Onboarding-Ablauf liest 20–40 Dateien, bevor der Agent nützliche Arbeit leisten kann — das sind 10–30k Tokens für das Verstehen statt für das Lösen des eigentlichen Problems des Benutzers.

ProjectLens ersetzt diese Orientierungsphase durch einen **einzigen Scan** (unter 100 ms), der einen tokenbegrenzten, framework-bewussten Kontextblock erzeugt. Der Agent liest **eine Kapsel** statt Dutzender Dateien. Allein durch Orientierungseinsparungen sinkt der Tokenverbrauch um 70–90% — und die 5 Session-Hooks stapeln weitere Einsparungen darauf.

**Der Kompromiss:** deterministische strukturelle Extraktion (schnell, kostenlos, framework-bewusst) statt semantischer Vektorsuche (langsamer, Embedding-Kosten, generisch). ProjectLens arbeitet neben semantischen Tools wie Cursor's `@codebase` und Sourcegraph Cody — nicht gegen sie. Verwenden Sie ProjectLens für sofortige Orientierung; greifen Sie zur semantischen Suche, wenn der Agent etwas Spezifisches nach Bedeutung statt nach Struktur finden muss.
...
📖 Vollständige Schritt-für-Schritt-Anleitung in **[`USER-INSTALL.md`](../../USER-INSTALL.md)**
---

## Funktionsweise

Die Scan-Engine durchläuft fünf Phasen pro Aufruf:
...
Typische Laufzeit: **30 ms** bei mittleren Projekten, **113 ms** bei einem 500-Datei-Projekt. Der Scan liest Dateien zwischen Phasen nicht erneut.

**Zwei Artefakte pro Scan:**
...
## Installation nach Tool (4 Kanäle)

Vier Distributionskanäle teilen sich dieselbe Scan-Engine. Wählen Sie den, der zu Ihrem Tool passt.

### Channel 1 — Claude Code / Cowork plugin (recommended)

Das volle Erlebnis: alle 5 Hooks aktiv, Slash-Befehle, Statusleiste, Memory-Loader.
...
Reiner stdlib JSON-RPC 2.0 stdio-Server. Kein `pip install`-Schritt für den Server selbst nötig — er läuft direkt aus dem geklonten Repo mit nur der Python-Standardbibliothek.
...
## Adaptive Stufen — T1 / T2 / T3

ProjectLens wählt automatisch die richtige Tiefe. Überschreiben Sie mit `--tier T1|T2|T3` nur, wenn Sie einen starken Grund haben.
...
Überschreibungshinweise im Chat — ProjectLens liest die Absicht:
...
## Framework-Abdeckung — 30 Adapter in 8 Paketen

30 Adapter über 8 Pakete. Jeder Adapter umfasst ca. 80–120 Codezeilen und wird nur lazy geladen, wenn seine Framework-Signatur dem Projekt entspricht.
...
Ein einziger ProjectLens-Scan in einem KI-Entwicklungsprojekt bringt jedes Glied der Kette zum Vorschein: von rohen Notebooks über Training, Modellierung, Embeddings, agentische Orchestrierung, UI-Komponenten bis hin zu Produktionsdeployments — alles in **einer Kapsel**, im Budget.

---

## Session-Hooks — 5 Produktions-Hooks

5 Hooks wirken zusammen in einer Claude Code-Sitzung. Token-Einsparungen stapeln sich über die Lebensdauer der Sitzung.
...
**Cowork-Einschränkung:** Nur SessionStart wird in Coworks Hook-Fläche ausgelöst. Die Scan-Engine, Kapselgenerierung und `/projectlens compact` funktionieren weiterhin — aber die 5 hook-getriebenen Optimierungen aktivieren sich nur in Claude Codes Terminal-CLI.

---

## Konversations-Kompaktor

Lange Sitzungen verbrauchen Kontext. Der Kompaktor gewinnt 8–25k Tokens in Sekunden zurück.
...
Der Kompaktor liest den von `activity_hook` aufgezeichneten Sitzungszustand und generiert eine `WORKING_CONTEXT.md`, die Folgendes zusammenfasst:
...
Leerzustands-Warnung: Wenn keine PostToolUse-Hooks ausgelöst wurden (typisch in Cowork), gibt der Kompaktor eine klare Diagnose aus, statt vorzugeben, Arbeit erfasst zu haben.

---

## Token-Ökonomie — konkrete Zahlen

Woher die Einsparungen kommen — konkrete Zahlen aus dem Produktionseinsatz:
...
**Kostenbeispiel (Opus-Preise, $15/Mtok Input):** Eine 4-stündige Coding-Sitzung, die zuvor ca. $3 an Input-Tokens kostete, fällt mit allen aktiven Hooks auf ca. $0,45–$0,90.

---

## Tests & Leistungsbudgets

**527 Unit-Tests** + **17 CI-erzwungene Performance- und Sicherheitsbudgets** laufen bei jedem Commit auf macOS / Linux / Windows × Python 3.9–3.12.

Gemessene Leistung auf synthetischen Projektgrößen:
...
Die Kapselgröße bleibt **unabhängig von der Projektgröße innerhalb des Stufenbudgets** — lineare Scan-Zeit, konstanter Output. Das ist der architektonische Graben.

In CI erzwungene Höchstgrenzen:
...
Das Hinzufügen von Adaptern kann keine dieser Werte verschlechtern. Pull Requests, die ein Budget verletzen, lassen CI scheitern.
...
**Nichts wird vom Gerät gesendet**, es sei denn, Sie führen explizit `/projectlens compact --llm` aus. Stats- und Memory-Dateien sind reines JSON — auditierbar, löschbar, keine PII.
...
Alle persistenten Oberflächen haben dokumentierte Umgebungsvariablen-Opt-outs:
...
Aufheben der Variable reaktiviert. Sonst ändert sich nichts — keine Plugin-Dateien verschoben, keine Daten verloren.
...
## Erweitern — eigenen Adapter schreiben

Das Adapter-SDK ist absichtlich klein gehalten. Jeder Adapter umfasst ca. 80–120 Codezeilen.
...
Der Vertrag, dem jeder Adapter folgen muss:
...
Legen Sie `.py`-Dateien in `<your-project>/.projectlens/frameworks/` ab. Sie werden bei jedem Scan automatisch erkannt, sobald Sie sich anmelden:

```bash
export PROJECTLENS_USER_ADAPTERS=1
```

Die Vertrauensentscheidung trifft Ihre Shell, nicht der gescannte Code — schützt Sie vor schädlichen Adaptern in nicht vertrauenswürdigen Repos.

---

## Vergleich mit Alternativen

ProjectLens ist **die leichte, deterministische Orientierungsschicht** im KI-Coding-Assistenz-Raum. Es ergänzt semantische Suchwerkzeuge, statt sie zu ersetzen.
...
A: Der Basis-Scan (Dateilauf, Spracherkennung, Symbole) funktioniert auf JS, TS, Go, Java, Rust, Ruby, PHP, C/C++ und ca. 20 weiteren Sprachen. Die 30 framework-spezifischen Adapter sind aktuell hauptsächlich Python-fokussiert, wobei Vue SFC + Tailwind + Docker Compose die JS/Web-Seite abdecken. JS/TS-Framework-Adapter (Next.js, Astro, SvelteKit, NestJS) sind auf der Roadmap.

**Q: Will it slow down my Claude Code session?**
A: Hook-Subprozess-Start ist auf 250 ms kalt begrenzt (typischerweise 20–30 ms warm). Hook-Ausgabe ist auf 500 Tokens pro Ereignis begrenzt. Beide Grenzen werden in CI erzwungen. Der Scan selbst läuft on-demand, nicht pro Prompt.

**Q: Does it send my code anywhere?**
A: Nein, es sei denn, Sie führen explizit `/projectlens compact --llm` aus UND haben `ANTHROPIC_API_KEY` gesetzt. Selbst dann wird nur die Zusammenfassung der Sitzungsaktivität (Dateipfade, Befehlsnamen, Testergebnisse) gesendet — niemals Dateiinhalte.

**Q: Is the capsule different from a README?**
A: Eine README sagt **Menschen**, was das Projekt macht. Die Kapsel sagt Ihrem **Agenten**, was das Projekt enthält: Routen, Modelle, Trainings-Loops, Vektor-Indizes, Deployments usw. Anderes Publikum, andere Ausgabe, beide haben ihren Platz.
...
**Q: Is it free?**
A: Ja — MIT-lizenziert. Kein Abo, keine Off-Device-Telemetrie, keine erforderlichen pip-Abhängigkeiten.
...
Stimmen Sie mit GitHub-Issues ab — was sollte zuerst kommen?
...
⭐ Markieren Sie dieses Repo mit einem Stern, wenn ProjectLens Ihnen Tokens spart. Eröffnen Sie ein Issue, wenn ein von Ihnen genutztes Framework nicht abgedeckt ist — die meisten Adapter umfassen ca. 100 Codezeilen, und wir werden es priorisieren.
