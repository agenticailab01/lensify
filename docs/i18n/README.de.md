...
15. [Erweitern — eigenen Adapter schreiben](#extending-lensify)
...
Lensify wählt automatisch die richtige Tiefe. Überschreiben Sie mit `--tier T1|T2|T3` nur, wenn Sie einen starken Grund haben.
...
Überschreibungshinweise im Chat — Lensify liest die Absicht:
...
Ein einziger Lensify-Scan in einem KI-Entwicklungsprojekt bringt jedes Glied der Kette zum Vorschein: von rohen Notebooks über Training, Modellierung, Embeddings, agentische Orchestrierung, UI-Komponenten bis hin zu Produktionsdeployments — alles in **einer Kapsel**, im Budget.
...
**Cowork-Einschränkung:** Nur SessionStart wird in Coworks Hook-Fläche ausgelöst. Die Scan-Engine, Kapselgenerierung und `/lensify compact` funktionieren weiterhin — aber die 5 hook-getriebenen Optimierungen aktivieren sich nur in Claude Codes Terminal-CLI.
...
**Nichts wird vom Gerät gesendet**, es sei denn, Sie führen explizit `/lensify compact --llm` aus. Stats- und Memory-Dateien sind reines JSON — auditierbar, löschbar, keine PII.
...
Legen Sie `.py`-Dateien in `<your-project>/.lensify/frameworks/` ab. Sie werden bei jedem Scan automatisch erkannt, sobald Sie sich anmelden:
...
Lensify ist **die leichte, deterministische Orientierungsschicht** im KI-Coding-Assistenz-Raum. Es ergänzt semantische Suchwerkzeuge, statt sie zu ersetzen.
...
A: Nein, es sei denn, Sie führen explizit `/lensify compact --llm` aus UND haben `ANTHROPIC_API_KEY` gesetzt. Selbst dann wird nur die Zusammenfassung der Sitzungsaktivität (Dateipfade, Befehlsnamen, Testergebnisse) gesendet — niemals Dateiinhalte.
...
⭐ Markieren Sie dieses Repo mit einem Stern, wenn Lensify Ihnen Tokens spart. Eröffnen Sie ein Issue, wenn ein von Ihnen genutztes Framework nicht abgedeckt ist — die meisten Adapter umfassen ca. 100 Codezeilen, und wir werden es priorisieren.
