...
15. [Extinde — scrie propriul adaptor](#extending-lensify)
...
Lensify înlocuiește acea fază de orientare cu **o singură scanare** (sub 100 ms) care produce un bloc de context limitat în tokeni, conștient de framework. Agentul citește **o capsulă** în loc de zeci de fișiere. Utilizarea tokenilor scade cu 70-90% doar din economiile de orientare.

**Compromisul:** extracție structurală deterministă (rapidă, gratuită, conștientă de framework) în loc de căutare vectorială semantică. Lensify lucrează alături de instrumentele semantice, nu împotriva lor.
...
Lensify alege automat profunzimea corectă.
...
O singură scanare Lensify pe un proiect AI scoate la suprafață fiecare verigă din lanț.
...
**Nimic nu este trimis în afara dispozitivului** decât dacă rulați explicit `/lensify compact --llm`.
...
Lensify este **stratul ușor și determinist de orientare** în spațiul asistenților de codare AI.
...
R: Nu, decât dacă rulați explicit `/lensify compact --llm` ȘI aveți `ANTHROPIC_API_KEY` setat.
...
⭐ Marcați cu stea acest repo dacă Lensify vă economisește tokeni.
