...
15. [Laajenna — kirjoita oma adapterisi](#extending-lensify)
...
Lensify korvaa tuon suunnistautumisvaiheen **yhdellä skannauksella** (alle 100 ms), joka tuottaa token-rajoitetun, framework-tietoisen konteksti-lohkon. Agentti lukee **yhden kapselin** kymmenien tiedostojen sijaan. Tokenien käyttö pienenee 70–90% jo pelkästään suunnistautumissäästöjen ansiosta.

**Kompromissi:** deterministinen rakenteellinen poiminta semanttisen vektorihaun sijaan. Lensify toimii semanttisten työkalujen rinnalla, ei niitä vastaan.
...
Lensify valitsee oikean syvyyden automaattisesti.
...
Yksi Lensify-skannaus AI-kehitysprojektissa tuo esiin ketjun jokaisen linkin.
...
**Mitään ei lähetetä laitteen ulkopuolelle**, ellet nimenomaisesti aja `/lensify compact --llm`.
...
Lensify on **kevyt, deterministinen suunnistautumiskerros** AI-koodausassistenttiavaruudessa.
...
V: Ei, ellet nimenomaisesti aja `/lensify compact --llm` JA ole asettanut `ANTHROPIC_API_KEY`:ta.
...
⭐ Anna tähti tälle repolle, jos Lensify säästää sinulta tokeneita.
