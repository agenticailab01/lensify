...
1. [Dlaczego Lensify](#why-lensify)
...
15. [Rozszerz — napisz własny adapter](#extending-lensify)
...
Lensify zastępuje tę fazę orientacji **pojedynczym skanowaniem** (poniżej 100 ms), które tworzy ograniczony tokenowo, świadomy frameworków blok kontekstu. Agent czyta **jedną kapsułę** zamiast dziesiątek plików. Użycie tokenów spada o 70–90% tylko z oszczędności orientacji — a 5 hooków sesji nawarstwia dodatkowe oszczędności.

**Kompromis, który robi:** deterministyczna ekstrakcja strukturalna (szybka, darmowa, świadoma frameworków) zamiast semantycznego wyszukiwania wektorowego (wolniejsze, koszt embeddingu, ogólne). Lensify działa obok narzędzi semantycznych jak Cursor `@codebase` i Sourcegraph Cody — nie przeciwko nim. Użyj Lensify do natychmiastowej orientacji; sięgnij po wyszukiwanie semantyczne, gdy agent musi znaleźć coś konkretnego po znaczeniu, a nie strukturze.
...
Lensify automatycznie wybiera właściwą głębokość. Nadpisz przez `--tier T1|T2|T3` tylko gdy masz mocny powód.
...
Wskazówki nadpisania w czacie — Lensify czyta intencję:
...
Jedno skanowanie Lensify projektu AI-dev wydobywa każdy łańcuch w sekwencji: od surowych notebooków przez trening, modelowanie, embeddingi, orkiestrację agentyczną, komponenty UI do wdrożeń produkcyjnych — wszystko w **jednej kapsule**, w budżecie.
...
**Ograniczenie Cowork:** tylko SessionStart odpala w powierzchni hooków Cowork. Silnik skanowania, generowanie kapsuły i `/lensify compact` nadal działają — ale 5 optymalizacji opartych na hookach aktywuje się tylko w terminalowym CLI Claude Code.
...
**Nic nie jest wysyłane poza urządzenie**, chyba że jawnie uruchomisz `/lensify compact --llm`. Pliki statystyk i pamięci są zwykłym JSON — audytowalne, usuwalne, bez PII.
...
Umieść pliki `.py` w `<your-project>/.lensify/frameworks/`. Są auto-odkrywane na skanowanie po opt-in:
...
Lensify to **lekka, deterministyczna warstwa orientacyjna** w przestrzeni asystentów kodowania AI. Uzupełnia narzędzia wyszukiwania semantycznego, zamiast je zastępować.
...
O: Nie, chyba że jawnie uruchomisz `/lensify compact --llm` ORAZ masz ustawione `ANTHROPIC_API_KEY`. Nawet wtedy wysyłane jest tylko podsumowanie aktywności sesji (ścieżki plików, nazwy komend, wyniki testów) — nigdy zawartość plików.
...
⭐ Daj gwiazdkę temu repo, jeśli Lensify oszczędza ci tokeny. Otwórz issue, jeśli framework, którego używasz, nie jest pokryty — większość adapterów to ~100 LOC i ustawimy priorytet.
