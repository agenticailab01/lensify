...
> 🌐 **Polski** — Powrót do angielskiego: [English](../../README.md)
...
> **Adaptacyjna soczewka projektu pojedynczego skanowania + kapsuła kontekstu zoptymalizowana pod kątem tokenów dla agentów kodowania AI.** Zmniejsza tokeny orientacji o 70–90%. Świadomość frameworków w całym cyklu życia rozwoju AI. Czysta biblioteka standardowa. Licencja MIT.

**📖 Przeczytaj w swoim języku:** 🇬🇧 [← Powrót do angielskiego](../../README.md) · 🇨🇳 [简体中文](README.zh-CN.md) · 🇹🇼 [繁體中文](README.zh-TW.md) · 🇯🇵 [日本語](README.ja.md) · 🇰🇷 [한국어](README.ko.md) · 🇩🇪 [Deutsch](README.de.md) · 🇫🇷 [Français](README.fr.md) · 🇪🇸 [Español](README.es.md) · 🇮🇳 [हिन्दी](README.hi.md) · 🇧🇷 [Português](README.pt.md) · 🇷🇺 [Русский](README.ru.md) · 🇸🇦 [العربية](README.ar.md) · 🇮🇹 [Italiano](README.it.md) · 🇳🇱 [Nederlands](README.nl.md) · 🇹🇷 [Türkçe](README.tr.md) · 🇺🇦 [Українська](README.uk.md) · 🇻🇳 [Tiếng Việt](README.vi.md) · 🇮🇩 [Bahasa Indonesia](README.id.md) · 🇸🇪 [Svenska](README.sv.md) · 🇬🇷 [Ελληνικά](README.el.md) · 🇷🇴 [Română](README.ro.md) · 🇨🇿 [Čeština](README.cs.md) · 🇫🇮 [Suomi](README.fi.md) · 🇩🇰 [Dansk](README.da.md) · 🇳🇴 [Norsk](README.no.md) · 🇭🇺 [Magyar](README.hu.md) · 🇹🇭 [ภาษาไทย](README.th.md) · 🇺🇿 [O'zbekcha](README.uz.md)

---

## Spis treści

1. [Dlaczego ProjectLens](#why-projectlens)
2. [W skrócie](#at-a-glance)
3. [Szybki start](#quick-start)
4. [Jak to działa](#how-it-works)
5. [Instalacja według narzędzia (4 kanały)](#installation-by-tool)
6. [Adaptacyjne poziomy — T1 / T2 / T3](#adaptive-tiers)
7. [Pokrycie frameworków — 30 adapterów w 8 pakietach](#framework-coverage)
8. [Hooki sesji — 5 hooków produkcyjnych](#session-hooks)
9. [Kompaktor konwersacji](#conversation-compactor)
10. [Ekonomia tokenów — konkretne liczby](#token-economics)
11. [Testy i budżety wydajności](#tests--performance)
12. [Bezpieczeństwo i zarządzanie](#security--governance)
13. [Zmienne środowiskowe konfiguracji](#configuration)
14. [Struktura projektu](#project-structure)
15. [Rozszerz — napisz własny adapter](#extending-projectlens)
16. [Porównanie z alternatywami](#comparison)
17. [FAQ](#faq)
18. [Mapa drogowa](#roadmap)
19. [Wkład i licencja](#contributing--license)

---

## Dlaczego ProjectLens

Nowoczesne agenty kodowania AI mają problem z oknem kontekstowym: im większy projekt, tym więcej tokenów spalają tylko na **orientację**. Typowy onboarding czyta 20–40 plików, zanim agent może wykonać użyteczną pracę — to 10–30k tokenów wydanych na rozumienie, a nie na rozwiązanie rzeczywistego problemu użytkownika.

ProjectLens zastępuje tę fazę orientacji **pojedynczym skanowaniem** (poniżej 100 ms), które tworzy ograniczony tokenowo, świadomy frameworków blok kontekstu. Agent czyta **jedną kapsułę** zamiast dziesiątek plików. Użycie tokenów spada o 70–90% tylko z oszczędności orientacji — a 5 hooków sesji nawarstwia dodatkowe oszczędności.

**Kompromis, który robi:** deterministyczna ekstrakcja strukturalna (szybka, darmowa, świadoma frameworków) zamiast semantycznego wyszukiwania wektorowego (wolniejsze, koszt embeddingu, ogólne). ProjectLens działa obok narzędzi semantycznych jak Cursor `@codebase` i Sourcegraph Cody — nie przeciwko nim. Użyj ProjectLens do natychmiastowej orientacji; sięgnij po wyszukiwanie semantyczne, gdy agent musi znaleźć coś konkretnego po znaczeniu, a nie strukturze.
...
📖 Pełne instrukcje krok po kroku w **[`USER-INSTALL.md`](../../USER-INSTALL.md)**
---

## Jak to działa

Silnik skanowania uruchamia pięć faz na wywołanie:
...
Typowy czas wykonania: **30 ms** na średnich projektach, **113 ms** na projekcie 500-plikowym. Skanowanie nigdy nie odczytuje plików ponownie między fazami.

**Dwa artefakty na skanowanie:**
...
## Instalacja według narzędzia (4 kanały)

Cztery kanały dystrybucji dzielą ten sam silnik skanowania. Wybierz ten, który pasuje do twojego narzędzia.

### Channel 1 — Claude Code / Cowork plugin (recommended)

Pełne doświadczenie: wszystkie 5 hooków odpala, slash commands, statusline, memory loader.
...
Czysty stdlib serwer JSON-RPC 2.0 stdio. Nie wymaga kroku `pip install` dla samego serwera — działa bezpośrednio z sklonowanego repo używając tylko standardowej biblioteki Pythona.
...
## Adaptacyjne poziomy — T1 / T2 / T3

ProjectLens automatycznie wybiera właściwą głębokość. Nadpisz przez `--tier T1|T2|T3` tylko gdy masz mocny powód.
...
Wskazówki nadpisania w czacie — ProjectLens czyta intencję:
...
## Pokrycie frameworków — 30 adapterów w 8 pakietach

30 adapterów w 8 pakietach. Każdy adapter to ~80–120 LOC, ładowany leniwie tylko gdy jego sygnatura frameworku pasuje do projektu.
...
Jedno skanowanie ProjectLens projektu AI-dev wydobywa każdy łańcuch w sekwencji: od surowych notebooków przez trening, modelowanie, embeddingi, orkiestrację agentyczną, komponenty UI do wdrożeń produkcyjnych — wszystko w **jednej kapsule**, w budżecie.

---

## Hooki sesji — 5 hooków produkcyjnych

5 hooków łączy się w sesji Claude Code. Oszczędności tokenów nawarstwiają się przez całą długość sesji.
...
**Ograniczenie Cowork:** tylko SessionStart odpala w powierzchni hooków Cowork. Silnik skanowania, generowanie kapsuły i `/projectlens compact` nadal działają — ale 5 optymalizacji opartych na hookach aktywuje się tylko w terminalowym CLI Claude Code.

---

## Kompaktor konwersacji

Długie sesje zjadają kontekst. Kompaktor odzyskuje 8–25k tokenów w sekundach.
...
Kompaktor czyta stan sesji zapisany przez `activity_hook` i generuje `WORKING_CONTEXT.md` podsumowujący:
...
Ostrzeżenie pustego stanu: jeśli żadne hooki PostToolUse nie odpaliły (typowe w Cowork), kompaktor wypisuje wyraźną diagnozę zamiast udawać, że uchwycił pracę.

---

## Ekonomia tokenów — konkretne liczby

Skąd biorą się oszczędności — konkretne liczby z używania produkcyjnego:
...
**Przykład kosztu (cennik Opus, $15/Mtok wejście):** 4-godzinna sesja kodowania, która wcześniej kosztowała ~$3 w tokenach wejściowych spada do ~$0.45–$0.90 z wszystkimi aktywnymi hookami.

---

## Testy i budżety wydajności

**527 testów jednostkowych** + **17 budżetów wydajności i bezpieczeństwa wymuszanych przez CI** uruchamia się przy każdym commit na macOS / Linux / Windows × Python 3.9–3.12.

Zmierzona wydajność na syntetycznych rozmiarach projektów:
...
Rozmiar kapsuły pozostaje **ograniczony budżetem poziomu niezależnie od rozmiaru projektu** — liniowy czas skanowania, stałe wyjście. To architektoniczna fosa.

Twarde limity wymuszane w CI:
...
Dodawanie adapterów nie może cofnąć żadnego z nich. Pull Requesty naruszające budżet powodują niepowodzenie CI.
...
**Nic nie jest wysyłane poza urządzenie**, chyba że jawnie uruchomisz `/projectlens compact --llm`. Pliki statystyk i pamięci są zwykłym JSON — audytowalne, usuwalne, bez PII.
...
Wszystkie trwałe powierzchnie mają udokumentowane opt-outy zmiennych środowiskowych:
...
Odznacz, aby ponownie włączyć. Nic innego się nie zmienia — żadne pliki pluginu nie są przenoszone, żadne dane nie są tracone.
...
## Rozszerz — napisz własny adapter

SDK adaptera jest celowo małe. Każdy adapter to ~80–120 LOC.
...
Kontrakt, którego każdy adapter musi przestrzegać:
...
Umieść pliki `.py` w `<your-project>/.projectlens/frameworks/`. Są auto-odkrywane na skanowanie po opt-in:

```bash
export PROJECTLENS_USER_ADAPTERS=1
```

Decyzja zaufania zachodzi w twojej powłoce, nie w skanowanym kodzie — chroniąc cię przed złośliwymi adapterami w niezaufanych repo.

---

## Porównanie z alternatywami

ProjectLens to **lekka, deterministyczna warstwa orientacyjna** w przestrzeni asystentów kodowania AI. Uzupełnia narzędzia wyszukiwania semantycznego, zamiast je zastępować.
...
O: Skanowanie podstawowe (przejście plików, wykrywanie języka, symbole) działa na JS, TS, Go, Java, Rust, Ruby, PHP, C/C++ i ~20 innych językach. 30 adapterów specyficznych dla frameworku jest obecnie głównie zorientowanych na Python, z Vue SFC + Tailwind + Docker Compose pokrywającym stronę JS/web. Adaptery frameworków JS/TS (Next.js, Astro, SvelteKit, NestJS) są na mapie drogowej.

**Q: Will it slow down my Claude Code session?**
O: Start podprocesu hooka jest ograniczony do 250 ms na zimno (zazwyczaj 20–30 ms na gorąco). Wyjście hooka jest ograniczone do 500 tokenów na zdarzenie. Oba limity są wymuszane przez CI. Samo skanowanie działa na żądanie, nie na prompt.

**Q: Does it send my code anywhere?**
O: Nie, chyba że jawnie uruchomisz `/projectlens compact --llm` ORAZ masz ustawione `ANTHROPIC_API_KEY`. Nawet wtedy wysyłane jest tylko podsumowanie aktywności sesji (ścieżki plików, nazwy komend, wyniki testów) — nigdy zawartość plików.

**Q: Is the capsule different from a README?**
O: README mówi **ludziom**, co robi projekt. Kapsuła mówi twojemu **agentowi**, co projekt zawiera: trasy, modele, pętle treningowe, indeksy wektorowe, wdrożenia itp. Inna publiczność, inne wyjście, oba mają swoje miejsce.
...
**Q: Is it free?**
O: Tak — licencja MIT. Bez subskrypcji, bez telemetrii off-device, bez wymaganych zależności pip.
...
Głosuj przez issues GitHub — co powinno być pierwsze?
...
⭐ Daj gwiazdkę temu repo, jeśli ProjectLens oszczędza ci tokeny. Otwórz issue, jeśli framework, którego używasz, nie jest pokryty — większość adapterów to ~100 LOC i ustawimy priorytet.
