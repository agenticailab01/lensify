...
> 🌐 **Українська** — Повернутись до англійської: [English](../../README.md)
...
> **Адаптивна лінза проекту з одним скануванням + оптимізована за токенами контекстна капсула для AI-агентів кодування.** Зменшує токени орієнтації на 70–90%. Розпізнавання фреймворків протягом усього життєвого циклу розробки AI. Чиста стандартна бібліотека. Ліцензія MIT.

**📖 Читайте вашою мовою:** 🇬🇧 [← Повернутись до англійської](../../README.md) · 🇨🇳 [简体中文](README.zh-CN.md) · 🇹🇼 [繁體中文](README.zh-TW.md) · 🇯🇵 [日本語](README.ja.md) · 🇰🇷 [한국어](README.ko.md) · 🇩🇪 [Deutsch](README.de.md) · 🇫🇷 [Français](README.fr.md) · 🇪🇸 [Español](README.es.md) · 🇮🇳 [हिन्दी](README.hi.md) · 🇧🇷 [Português](README.pt.md) · 🇷🇺 [Русский](README.ru.md) · 🇸🇦 [العربية](README.ar.md) · 🇮🇹 [Italiano](README.it.md) · 🇵🇱 [Polski](README.pl.md) · 🇳🇱 [Nederlands](README.nl.md) · 🇹🇷 [Türkçe](README.tr.md) · 🇻🇳 [Tiếng Việt](README.vi.md) · 🇮🇩 [Bahasa Indonesia](README.id.md) · 🇸🇪 [Svenska](README.sv.md) · 🇬🇷 [Ελληνικά](README.el.md) · 🇷🇴 [Română](README.ro.md) · 🇨🇿 [Čeština](README.cs.md) · 🇫🇮 [Suomi](README.fi.md) · 🇩🇰 [Dansk](README.da.md) · 🇳🇴 [Norsk](README.no.md) · 🇭🇺 [Magyar](README.hu.md) · 🇹🇭 [ภาษาไทย](README.th.md) · 🇺🇿 [O'zbekcha](README.uz.md)

---

## Зміст

1. [Чому ProjectLens](#why-projectlens)
2. [Огляд](#at-a-glance)
3. [Швидкий старт](#quick-start)
4. [Як це працює](#how-it-works)
5. [Встановлення за інструментом (4 канали)](#installation-by-tool)
6. [Адаптивні рівні — T1 / T2 / T3](#adaptive-tiers)
7. [Покриття фреймворків — 30 адаптерів у 8 пакетах](#framework-coverage)
8. [Хуки сесії — 5 продакшен-хуків](#session-hooks)
9. [Компактор розмов](#conversation-compactor)
10. [Економіка токенів — конкретні цифри](#token-economics)
11. [Тести та бюджети продуктивності](#tests--performance)
12. [Безпека та управління](#security--governance)
13. [Змінні середовища конфігурації](#configuration)
14. [Структура проекту](#project-structure)
15. [Розширення — напишіть свій адаптер](#extending-projectlens)
16. [Порівняння з альтернативами](#comparison)
17. [FAQ](#faq)
18. [Дорожня карта](#roadmap)
19. [Внесок та ліцензія](#contributing--license)

---

## Чому ProjectLens

Сучасні AI-агенти кодування мають проблему контекстного вікна: чим більший проект, тим більше токенів вони витрачають лише на **орієнтацію**. Типовий потік онбордингу читає 20-40 файлів, перш ніж агент зможе виконувати корисну роботу — це 10-30k токенів, витрачених на розуміння, а не на вирішення реальної проблеми користувача.

ProjectLens замінює цю фазу орієнтації **єдиним скануванням** (менше 100 мс), яке створює обмежений за токенами, орієнтований на фреймворки блок контексту. Агент читає **одну капсулу** замість десятків файлів. Використання токенів падає на 70-90% лише завдяки заощадженням на орієнтації — і 5 хуків сесії додають додаткові заощадження зверху.

**Компроміс, на який воно йде:** детерміноване структурне витягування (швидке, безкоштовне, орієнтоване на фреймворки) замість семантичного векторного пошуку (повільніший, вартість ембедингу, загальний). ProjectLens працює поряд із семантичними інструментами, як-от `@codebase` від Cursor та Sourcegraph Cody — не проти них. Використовуйте ProjectLens для миттєвої орієнтації; вдавайтеся до семантичного пошуку, коли агенту потрібно знайти щось конкретне за значенням, а не за структурою.
...
📖 Повні покрокові інструкції в **[`USER-INSTALL.md`](../../USER-INSTALL.md)**
---

## Як це працює

Сканувальний рушій виконує п'ять фаз за виклик:
...
Типовий час виконання: **30 мс** на середніх проектах, **113 мс** на проекті з 500 файлів. Сканування ніколи не перечитує файли між фазами.

**Два артефакти на сканування:**
...
## Встановлення за інструментом (4 канали)

Чотири канали розповсюдження ділять один рушій сканування. Виберіть той, що відповідає вашому інструменту.

### Channel 1 — Claude Code / Cowork plugin (recommended)

Повний досвід: усі 5 хуків спрацьовують, slash-команди, статусний рядок, завантажувач пам'яті.
...
Чистий stdlib JSON-RPC 2.0 stdio сервер. Сам сервер не потребує кроку `pip install` — він запускається прямо з клонованого репо, використовуючи лише стандартну бібліотеку Python.
...
## Адаптивні рівні — T1 / T2 / T3

ProjectLens автоматично обирає правильну глибину. Перевизначайте через `--tier T1|T2|T3` лише за наявності вагомої причини.
...
Натяки перевизначення у чаті — ProjectLens читає намір:
...
## Покриття фреймворків — 30 адаптерів у 8 пакетах

30 адаптерів у 8 пакетах. Кожен адаптер — ~80–120 LOC, лінива загрузка лише коли підпис фреймворку відповідає проекту.
...
Одне сканування ProjectLens на AI-проекті виявляє кожну ланку ланцюга: від сирих ноутбуків через навчання, моделювання, ембединги, агентську оркестрацію, UI-компоненти до продакшен-розгортань — усе в **одній капсулі**, у межах бюджету.

---

## Хуки сесії — 5 продакшен-хуків

5 хуків складаються в сесії Claude Code. Заощадження токенів накопичуються протягом усього життя сесії.
...
**Обмеження Cowork:** лише SessionStart спрацьовує на хук-поверхні Cowork. Сканувальний рушій, генерація капсул та `/projectlens compact` усе ще працюють — але 5 хук-керованих оптимізацій активуються лише в термінальному CLI Claude Code.

---

## Компактор розмов

Довгі сесії з'їдають контекст. Компактор відновлює 8-25k токенів за секунди.
...
Компактор читає стан сесії, записаний `activity_hook`, і генерує `WORKING_CONTEXT.md`, що підсумовує:
...
Попередження порожнього стану: якщо жоден PostToolUse хук не спрацював (типово в Cowork), компактор видає чіткий діагноз замість того, щоб удавати, що захопив роботу.

---

## Економіка токенів — конкретні цифри

Звідки беруться заощадження — конкретні цифри з продакшен-використання:
...
**Приклад вартості (тариф Opus, $15/Mtok вхід):** 4-годинна сесія кодування, яка раніше коштувала ~$3 у вхідних токенах, знижується до ~$0,45–$0,90 з усіма активними хуками.

---

## Тести та бюджети продуктивності

**527 модульних тестів** + **17 примусово застосованих CI бюджетів продуктивності та безпеки** запускаються при кожному комміті на macOS / Linux / Windows × Python 3.9–3.12.

Виміряна продуктивність на синтетичних розмірах проектів:
...
Розмір капсули залишається **обмеженим бюджетом рівня незалежно від розміру проекту** — лінійний час сканування, постійний вихід. Це архітектурний рів.

Жорсткі обмеження, які примусово застосовуються в CI:
...
Додавання адаптерів не може зробити жоден з них регресуючим. Pull Requests, що порушують бюджет, провалюють CI.
...
**Нічого не надсилається за межі пристрою**, якщо ви не запустите явно `/projectlens compact --llm`. Файли статистики та пам'яті — це звичайний JSON: аудиторований, видаляється, без PII.

For governance — what contributions we accept and what we don't — see [`GOVERNANCE.md`](../../GOVERNANCE.md).

---

## Змінні середовища конфігурації

Усі постійні поверхні мають задокументовані опт-аути env vars:
...
Скиньте, щоб увімкнути знову. Більше нічого не змінюється — жодних переміщених файлів плагіна, жодних втрачених даних.
...
## Розширення — напишіть свій адаптер

SDK адаптера навмисно малий. Кожен адаптер — ~80–120 LOC.
...
Контракт, якого мусить дотримуватися кожен адаптер:
...
Помістіть файли `.py` у `<your-project>/.projectlens/frameworks/`. Після opt-in вони автовиявляються при кожному скануванні:

```bash
export PROJECTLENS_USER_ADAPTERS=1
```

Рішення довіри приймається у вашому shell, а не в коді, який сканується — захищаючи вас від шкідливих адаптерів у недовірених репо.

---

## Порівняння з альтернативами

ProjectLens — це **легкий, детермінований шар орієнтації** у просторі AI-асистентів кодування. Він доповнює інструменти семантичного пошуку, а не замінює їх.
...
В: Базове сканування (обхід файлів, виявлення мови, символи) працює на JS, TS, Go, Java, Rust, Ruby, PHP, C/C++ та ~20 інших мовах. 30 специфічних для фреймворку адаптерів зараз переважно орієнтовані на Python, із Vue SFC + Tailwind + Docker Compose, що покривають сторону JS/web. Адаптери фреймворків JS/TS (Next.js, Astro, SvelteKit, NestJS) у дорожній карті.

**Q: Will it slow down my Claude Code session?**
В: Запуск підпроцесу хука обмежений 250 мс у холодному режимі (зазвичай 20-30 мс у гарячому). Вихід хука обмежений 500 токенами на подію. Обидва обмеження примусово застосовуються CI. Саме сканування виконується за запитом, а не на промпт.

**Q: Does it send my code anywhere?**
В: Ні, якщо тільки ви явно не запустите `/projectlens compact --llm` І не встановите `ANTHROPIC_API_KEY`. Навіть тоді надсилається лише підсумок активності сесії (шляхи файлів, імена команд, результати тестів) — ніколи вміст файлів.

**Q: Is the capsule different from a README?**
В: README говорить **людям**, що робить проект. Капсула розказує вашому **агенту**, що містить проект: маршрути, моделі, тренувальні цикли, векторні індекси, розгортання тощо. Різна аудиторія, різний вивід, обидва мають своє місце.
...
**Q: Is it free?**
В: Так — ліцензія MIT. Без підписки, без off-device телеметрії, без обов'язкових pip-залежностей.
...
Голосуйте через GitHub issues — що має бути першим?
...
⭐ Зірочка цьому репо, якщо ProjectLens заощаджує вам токени. Відкрийте issue, якщо фреймворк, який ви використовуєте, не покривається — більшість адаптерів ~100 LOC, і ми пріоритизуватимемо.
