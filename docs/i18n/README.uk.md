...
15. [Розширення — напишіть свій адаптер](#extending-lensify)
...
Lensify замінює цю фазу орієнтації **єдиним скануванням** (менше 100 мс), яке створює обмежений за токенами, орієнтований на фреймворки блок контексту. Агент читає **одну капсулу** замість десятків файлів. Використання токенів падає на 70-90% лише завдяки заощадженням на орієнтації — і 5 хуків сесії додають додаткові заощадження зверху.

**Компроміс, на який воно йде:** детерміноване структурне витягування (швидке, безкоштовне, орієнтоване на фреймворки) замість семантичного векторного пошуку (повільніший, вартість ембедингу, загальний). Lensify працює поряд із семантичними інструментами, як-от `@codebase` від Cursor та Sourcegraph Cody — не проти них. Використовуйте Lensify для миттєвої орієнтації; вдавайтеся до семантичного пошуку, коли агенту потрібно знайти щось конкретне за значенням, а не за структурою.
...
Lensify автоматично обирає правильну глибину. Перевизначайте через `--tier T1|T2|T3` лише за наявності вагомої причини.
...
Натяки перевизначення у чаті — Lensify читає намір:
...
Одне сканування Lensify на AI-проекті виявляє кожну ланку ланцюга: від сирих ноутбуків через навчання, моделювання, ембединги, агентську оркестрацію, UI-компоненти до продакшен-розгортань — усе в **одній капсулі**, у межах бюджету.
...
**Обмеження Cowork:** лише SessionStart спрацьовує на хук-поверхні Cowork. Сканувальний рушій, генерація капсул та `/lensify compact` усе ще працюють — але 5 хук-керованих оптимізацій активуються лише в термінальному CLI Claude Code.
...
**Нічого не надсилається за межі пристрою**, якщо ви не запустите явно `/lensify compact --llm`. Файли статистики та пам'яті — це звичайний JSON: аудиторований, видаляється, без PII.
...
Помістіть файли `.py` у `<your-project>/.lensify/frameworks/`. Після opt-in вони автовиявляються при кожному скануванні:
...
Lensify — це **легкий, детермінований шар орієнтації** у просторі AI-асистентів кодування. Він доповнює інструменти семантичного пошуку, а не замінює їх.
...
В: Ні, якщо тільки ви явно не запустите `/lensify compact --llm` І не встановите `ANTHROPIC_API_KEY`. Навіть тоді надсилається лише підсумок активності сесії (шляхи файлів, імена команд, результати тестів) — ніколи вміст файлів.
...
⭐ Зірочка цьому репо, якщо Lensify заощаджує вам токени. Відкрийте issue, якщо фреймворк, який ви використовуєте, не покривається — більшість адаптерів ~100 LOC, і ми пріоритизуватимемо.
