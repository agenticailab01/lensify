...
5. [Installatie per tool (4 kanalen)](#installation-by-tool)
6. [Adaptieve niveaus — T1 / T2 / T3](#adaptive-tiers)
7. [Framework-dekking — 30 adapters in 8 packs](#framework-coverage)
8. [Sessie-hooks — 5 productie-hooks](#session-hooks)
9. [Gespreks-compactor](#conversation-compactor)
10. [Token-economie — concrete cijfers](#token-economics)
11. [Tests en prestatiebudgetten](#tests--performance)
...
17. [FAQ](#faq)
18. [Roadmap](#roadmap)
19. [Bijdragen en licentie](#contributing--license)
...
De scan-engine voert vijf fases per aanroep uit:
...
Typische runtime: **30 ms** op middelgrote projecten, **113 ms** op een 500-bestandsproject. De scan herleest nooit bestanden tussen fases.

**Twee artefacten per scan:**
...
## Installatie per tool (4 kanalen)

Vier distributiekanalen delen dezelfde scan-engine. Kies degene die bij je tool past.
...
Pure-stdlib JSON-RPC 2.0 stdio-server. Geen `pip install`-stap nodig voor de server zelf — het draait rechtstreeks vanuit de gekloonde repo met alleen de Python-standaardbibliotheek.
...
## Framework-dekking — 30 adapters in 8 packs

30 adapters over 8 packs. Elke adapter is ~80–120 LOC, lazy geladen alleen wanneer zijn frameworkhandtekening overeenkomt met het project.
...
## Sessie-hooks — 5 productie-hooks

5 hooks combineren over een Claude Code-sessie. Tokenbesparingen stapelen over de levensduur van de sessie.
...
De compactor leest de sessiestatus die door `activity_hook` is vastgelegd en genereert een `WORKING_CONTEXT.md` die het volgende samenvat:
...
Lege staat waarschuwing: als er geen PostToolUse-hooks zijn afgevuurd (typisch in Cowork), geeft de compactor een duidelijke diagnose in plaats van te doen alsof het werk heeft vastgelegd.

---

## Token-economie — concrete cijfers
...
**Kostenvoorbeeld (Opus-prijs, $15/Mtok input):** een coderingssessie van 4 uur die voorheen ~$3 aan input-tokens kostte, daalt naar ~$0,45-$0,90 met alle hooks actief.

---

## Tests en prestatiebudgetten

**527 unit tests** + **17 CI-afgedwongen prestatie- en beveiligingsbudgetten** draaien bij elke commit op macOS / Linux / Windows × Python 3.9-3.12.

Gemeten prestaties op synthetische projectgroottes:
...
Capsulegrootte blijft **begrensd door tier-budget ongeacht projectgrootte** — lineaire scantijd, constante output. Dat is de architectonische gracht.

Harde plafonds afgedwongen in CI:
...
Het toevoegen van adapters kan geen van deze laten teruglopen. Pull Requests die een budget schenden laten CI falen.
...
Alle persistente oppervlakken hebben gedocumenteerde env var opt-outs:
...
## Uitbreiden — schrijf je eigen adapter

De adapter-SDK is opzettelijk klein. Elke adapter is ~80-120 LOC.
...
Het contract dat elke adapter moet volgen:
...
A: De basisscan (bestandswandeling, taaldetectie, symbolen) werkt op JS, TS, Go, Java, Rust, Ruby, PHP, C/C++ en ~20 andere talen. De 30 framework-specifieke adapters zijn momenteel voornamelijk Python-gericht, met Vue SFC + Tailwind + Docker Compose voor de JS/web-kant. JS/TS framework-adapters (Next.js, Astro, SvelteKit, NestJS) staan op de roadmap.
