...
15. [Επέκταση — γράψτε τον δικό σας προσαρμογέα](#extending-lensify)
...
Το Lensify αντικαθιστά αυτή τη φάση προσανατολισμού με μια **μοναδική σάρωση** (κάτω από 100 ms) που παράγει ένα μπλοκ περιβάλλοντος με όριο tokens και επίγνωση frameworks. Ο πράκτορας διαβάζει **μία κάψουλα** αντί για δεκάδες αρχεία. Η χρήση tokens πέφτει 70–90% μόνο από τις εξοικονομήσεις προσανατολισμού.

**Η αντιστάθμιση:** ντετερμινιστική δομική εξαγωγή (γρήγορη, δωρεάν, με επίγνωση frameworks) αντί σημασιολογικής αναζήτησης διανυσμάτων. Το Lensify συνεργάζεται με σημασιολογικά εργαλεία, δεν τα αντικαθιστά.
...
Το Lensify επιλέγει αυτόματα το σωστό βάθος.
...
Μια ενιαία σάρωση Lensify σε ένα έργο AI ανάπτυξης αναδεικνύει κάθε κρίκο στην αλυσίδα.
...
**Τίποτα δεν αποστέλλεται εκτός συσκευής** εκτός εάν εκτελέσετε ρητά `/lensify compact --llm`.
...
Το Lensify είναι **το ελαφρύ, ντετερμινιστικό επίπεδο προσανατολισμού**.
...
Α: Όχι, εκτός εάν εκτελέσετε ρητά `/lensify compact --llm` ΚΑΙ έχετε ορίσει `ANTHROPIC_API_KEY`.
...
⭐ Αστεροειδώστε αυτό το repo εάν το Lensify σας εξοικονομεί tokens.
