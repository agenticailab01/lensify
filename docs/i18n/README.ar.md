...
1. [لماذا ProjectLens](#why-projectlens)
...
15. [التوسيع — اكتب محولك الخاص](#extending-projectlens)
...
تستبدل ProjectLens مرحلة التوجيه تلك **بفحص واحد** (دون 100 مللي ثانية) ينتج كتلة سياق محدودة بالرموز وواعية بأطر العمل. يقرأ الوكيل **كبسولة واحدة** بدلاً من عشرات الملفات. ينخفض استخدام الرموز بنسبة 70–90٪ من توفيرات التوجيه وحدها — و 5 خطافات الجلسة تضيف توفيرات إضافية فوقها.

**المقايضة التي يقوم بها:** استخراج هيكلي حتمي (سريع، مجاني، واعٍ بأطر العمل) بدلاً من البحث الشعاعي الدلالي (أبطأ، تكلفة تضمين، عام). تعمل ProjectLens جنبًا إلى جنب مع أدوات دلالية مثل `@codebase` من Cursor و Sourcegraph Cody — وليس ضدها. استخدم ProjectLens للتوجيه الفوري؛ الجأ إلى البحث الدلالي عندما يحتاج الوكيل إلى العثور على شيء محدد بالمعنى وليس بالهيكل.
...
تختار ProjectLens العمق الصحيح تلقائيًا. تجاوز بـ `--tier T1|T2|T3` فقط عندما يكون لديك سبب قوي.
...
تلميحات التجاوز في الدردشة — تقرأ ProjectLens القصد:
...
فحص ProjectLens واحد على مشروع تطوير AI يظهر كل حلقة في السلسلة: من الدفاتر الخام عبر التدريب، النمذجة، التضمينات، التنسيق الوكيل، مكونات UI، ونشر الإنتاج — كل ذلك في **كبسولة واحدة**، تحت الميزانية.
...
**قيود Cowork:** فقط SessionStart يطلق في سطح خطافات Cowork. محرك الفحص، إنشاء الكبسولة، و `/projectlens compact` لا تزال تعمل — لكن التحسينات المدفوعة بـ 5 خطافات تنشط فقط في CLI طرفية Claude Code.
...
**لا شيء يُرسل خارج الجهاز** إلا إذا قمت بتشغيل `/projectlens compact --llm` بشكل صريح. ملفات الإحصائيات والذاكرة هي JSON عادي — قابلة للتدقيق، قابلة للحذف، بدون PII.
...
ضع ملفات `.py` في `<your-project>/.projectlens/frameworks/`. يتم اكتشافها تلقائيًا لكل فحص بمجرد أن تشترك:
...
ProjectLens هي **طبقة التوجيه الخفيفة الحتمية** في فضاء مساعدي البرمجة بالذكاء الاصطناعي. تكمل أدوات البحث الدلالي بدلاً من استبدالها.
...
ج: لا، إلا إذا قمت بتشغيل `/projectlens compact --llm` بشكل صريح **و** تم تعيين `ANTHROPIC_API_KEY`. حتى في هذه الحالة، يتم إرسال ملخص نشاط الجلسة فقط (مسارات الملفات، أسماء الأوامر، نتائج الاختبارات) — أبدًا محتويات الملفات.
...
⭐ ضع نجمة لهذا المستودع إذا كانت ProjectLens توفر لك الرموز. افتح قضية إذا كان إطار العمل الذي تستخدمه غير مغطى — معظم المحولات ~100 سطر من التعليمات البرمجية وسنعطيها الأولوية.
