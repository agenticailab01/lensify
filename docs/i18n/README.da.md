...
> 🌐 **Dansk** — Tilbage til engelsk: [English](../../README.md)
...
2. [Et overblik](#at-a-glance)
3. [Hurtig start](#quick-start)
4. [Sådan fungerer det](#how-it-works)
5. [Installation per værktøj (4 kanaler)](#installation-by-tool)
6. [Adaptive niveauer — T1 / T2 / T3](#adaptive-tiers)
7. [Frameworkdækning — 30 adaptere i 8 pakker](#framework-coverage)
8. [Sessionshooks — 5 produktionshooks](#session-hooks)
9. [Konversationskompaktor](#conversation-compactor)
10. [Tokenøkonomi — konkrete tal](#token-economics)
11. [Tests og performancebudgetter](#tests--performance)
12. [Sikkerhed og styring](#security--governance)
13. [Konfigurations-miljøvariabler](#configuration)
14. [Projektstruktur](#project-structure)
15. [Udvid — skriv din egen adapter](#extending-projectlens)
16. [Sammenligning med alternativer](#comparison)
17. [FAQ](#faq)
18. [Køreplan](#roadmap)
19. [Bidrag og licens](#contributing--license)

---

## Hvorfor ProjectLens

Moderne AI-kodningsagenter har et kontekstvinduesproblem: jo større projektet er, jo flere tokens brænder de bare på at **orientere sig selv**. En typisk onboarding-flow læser 20-40 filer, før agenten kan udføre nyttigt arbejde — det er 10-30k tokens brugt på at forstå, ikke på at løse brugerens faktiske problem.

ProjectLens erstatter denne orienteringsfase med en **enkelt scanning** (under 100 ms), der producerer en token-begrænset, framework-bevidst kontekstblok. Agenten læser **én kapsel** i stedet for dusinvis af filer. Tokenforbrug falder 70-90% alene fra orienteringsbesparelser.

**Kompromiset:** deterministisk strukturel udtræk i stedet for semantisk vektorsøgning. ProjectLens arbejder sammen med semantiske værktøjer, ikke imod dem.
...
📖 Komplette trin-for-trin instruktioner i **[`USER-INSTALL.md`](../../USER-INSTALL.md)**
---

## Sådan fungerer det

Scanningsmotoren kører fem faser pr. opkald:
...
Typisk køretid: **30 ms** på mellemstore projekter, **113 ms** på et 500-fils projekt.

**To artefakter pr. scanning:**
...
## Installation per værktøj (4 kanaler)

Fire distributionskanaler deler den samme scanningsmotor.

### Channel 1 — Claude Code / Cowork plugin (recommended)

Den fulde oplevelse: alle 5 hooks affyrer, slash-kommandoer, statusline, memory loader.
...
Ren stdlib JSON-RPC 2.0 stdio-server. Ingen `pip install`-trin nødvendigt for selve serveren — den kører direkte fra det klonede repo ved kun at bruge Python-standardbiblioteket.
...
## Adaptive niveauer — T1 / T2 / T3

ProjectLens vælger automatisk den rigtige dybde.
...
Overskrivnings-hints i chatten:
...
## Frameworkdækning — 30 adaptere i 8 pakker

30 adaptere på tværs af 8 pakker. Hver adapter er ~80–120 LOC.
...
En enkelt ProjectLens-scanning på et AI-udviklingsprojekt afslører hvert led i kæden.

---

## Sessionshooks — 5 produktionshooks

5 hooks komponerer på tværs af en Claude Code-session. Tokenbesparelser stables.
...
**Cowork-begrænsning:** kun SessionStart affyrer i Coworks hook-overflade.

---

## Konversationskompaktor

Lange sessioner spiser kontekst. Kompaktoren genvinder 8-25k tokens på sekunder.
...
Kompaktoren læser sessionstilstanden og genererer en `WORKING_CONTEXT.md`:
...
Tom tilstandsadvarsel: hvis ingen PostToolUse-hooks affyrede, udsender kompaktoren en klar diagnose.

---

## Tokenøkonomi — konkrete tal

Hvor besparelserne kommer fra — konkrete tal:
...
**Omkostningseksempel (Opus-priser, $15/Mtok input):** en 4-timers kodningssession, der før kostede ~$3, falder til ~$0,45–$0,90.

---

## Tests og performancebudgetter

**527 enhedstests** + **17 CI-håndhævede ydeevne- og sikkerhedsbudgetter** kører på hver commit.
...
**Intet sendes off-device**, medmindre du eksplicit kører `/projectlens compact --llm`.

For governance — what contributions we accept and what we don't — see [`GOVERNANCE.md`](../../GOVERNANCE.md).

---

## Konfigurations-miljøvariabler
...
ProjectLens er **det lette, deterministiske orienteringslag** i AI-kodningsassistentrummet.
...
S: Grundscanningen fungerer på JS, TS, Go, Java, Rust og ~20 andre sprog.
...
S: Nej, medmindre du eksplicit kører `/projectlens compact --llm` OG har `ANTHROPIC_API_KEY` sat.
...
**Q: Is it free?**
S: Ja — MIT-licenseret. Intet abonnement, ingen telemetri.
...
Stem med GitHub issues — hvad skal komme først?
...
⭐ Stjernemarker dette repo, hvis ProjectLens sparer dig tokens.
