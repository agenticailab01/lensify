...
> 🌐 **Türkçe** — İngilizceye dön: [English](../../README.md)
...
> **AI kodlama ajanları için tek taramalı uyarlanabilir proje merceği + token optimize edilmiş bağlam kapsülü.** Yönelim tokenlerini %70–90 azaltır. Tüm AI geliştirme yaşam döngüsünde framework farkındalığı. Saf standart kütüphane. MIT lisanslı.

**📖 Kendi dilinizde okuyun:** 🇬🇧 [← İngilizceye dön](../../README.md) · 🇨🇳 [简体中文](README.zh-CN.md) · 🇹🇼 [繁體中文](README.zh-TW.md) · 🇯🇵 [日本語](README.ja.md) · 🇰🇷 [한국어](README.ko.md) · 🇩🇪 [Deutsch](README.de.md) · 🇫🇷 [Français](README.fr.md) · 🇪🇸 [Español](README.es.md) · 🇮🇳 [हिन्दी](README.hi.md) · 🇧🇷 [Português](README.pt.md) · 🇷🇺 [Русский](README.ru.md) · 🇸🇦 [العربية](README.ar.md) · 🇮🇹 [Italiano](README.it.md) · 🇵🇱 [Polski](README.pl.md) · 🇳🇱 [Nederlands](README.nl.md) · 🇺🇦 [Українська](README.uk.md) · 🇻🇳 [Tiếng Việt](README.vi.md) · 🇮🇩 [Bahasa Indonesia](README.id.md) · 🇸🇪 [Svenska](README.sv.md) · 🇬🇷 [Ελληνικά](README.el.md) · 🇷🇴 [Română](README.ro.md) · 🇨🇿 [Čeština](README.cs.md) · 🇫🇮 [Suomi](README.fi.md) · 🇩🇰 [Dansk](README.da.md) · 🇳🇴 [Norsk](README.no.md) · 🇭🇺 [Magyar](README.hu.md) · 🇹🇭 [ภาษาไทย](README.th.md) · 🇺🇿 [O'zbekcha](README.uz.md)

---

## İçindekiler

1. [Neden ProjectLens](#why-projectlens)
2. [Bir bakışta](#at-a-glance)
3. [Hızlı başlangıç](#quick-start)
4. [Nasıl çalışır](#how-it-works)
5. [Araca göre kurulum (4 kanal)](#installation-by-tool)
6. [Uyarlanabilir katmanlar — T1 / T2 / T3](#adaptive-tiers)
7. [Framework kapsamı — 8 paketteki 30 adaptör](#framework-coverage)
8. [Oturum kancaları — 5 üretim kancası](#session-hooks)
9. [Konuşma sıkıştırıcı](#conversation-compactor)
10. [Token ekonomisi — somut sayılar](#token-economics)
11. [Testler ve performans bütçeleri](#tests--performance)
12. [Güvenlik ve yönetişim](#security--governance)
13. [Yapılandırma ortam değişkenleri](#configuration)
14. [Proje yapısı](#project-structure)
15. [Genişlet — kendi adaptörünüzü yazın](#extending-projectlens)
16. [Alternatiflerle karşılaştırma](#comparison)
17. [SSS](#faq)
18. [Yol haritası](#roadmap)
19. [Katkı ve lisans](#contributing--license)

---

## Neden ProjectLens

Modern AI kodlama ajanlarının bir bağlam penceresi sorunu var: proje ne kadar büyükse, sadece **kendini yönlendirmek** için o kadar fazla token yakarlar. Tipik bir onboarding akışı, ajan yararlı iş yapabilmeden önce 20–40 dosya okur — bu, kullanıcının gerçek sorununu çözmek yerine anlamak için harcanan 10–30k token demektir.

ProjectLens, bu yönlendirme aşamasını **tek bir tarama** (100 ms altı) ile değiştirir; bu tarama, token ile sınırlı, framework-bilinçli bir bağlam bloğu üretir. Ajan, düzinelerce dosya yerine **bir kapsül** okur. Token kullanımı, sadece yönlendirme tasarruflarından %70–90 düşer — ve 5 oturum kancası bunun üzerine ek tasarruflar ekler.

**Yaptığı ödün:** semantik vektör araması (daha yavaş, embedding maliyeti, jenerik) yerine deterministik yapısal çıkarma (hızlı, ücretsiz, framework-bilinçli). ProjectLens, Cursor'un `@codebase` ve Sourcegraph Cody gibi semantik araçların yanında çalışır — onlara karşı değil. Anlık yönlendirme için ProjectLens'i kullanın; ajan, yapı yerine anlam ile belirli bir şeyi bulması gerektiğinde semantik aramaya başvurun.
...
📖 Tam adım adım talimatlar şu dosyada **[`USER-INSTALL.md`](../../USER-INSTALL.md)**
---

## Nasıl çalışır

Tarama motoru, çağrı başına beş aşama çalıştırır:
...
Tipik çalışma süresi: orta projelerde **30 ms**, 500 dosyalı projede **113 ms**. Tarama, aşamalar arasında dosyaları asla tekrar okumaz.

**Tarama başına iki çıktı:**
...
## Araca göre kurulum (4 kanal)

Dört dağıtım kanalı aynı tarama motorunu paylaşır. Aracınıza uyanı seçin.

### Channel 1 — Claude Code / Cowork plugin (recommended)

Tam deneyim: tüm 5 kanca ateşlenir, slash komutları, statusline, memory loader.
...
Saf-stdlib JSON-RPC 2.0 stdio sunucusu. Sunucunun kendisi için `pip install` adımı gerekmez — sadece Python standart kütüphanesini kullanarak klonlanmış repo'dan doğrudan çalışır.
...
## Uyarlanabilir katmanlar — T1 / T2 / T3

ProjectLens doğru derinliği otomatik seçer. Yalnızca güçlü bir nedeniniz olduğunda `--tier T1|T2|T3` ile geçersiz kılın.
...
Sohbette geçersiz kılma ipuçları — ProjectLens niyeti okur:
...
## Framework kapsamı — 8 paketteki 30 adaptör

8 paket boyunca 30 adaptör. Her adaptör ~80–120 LOC, yalnızca framework imzası projeyle eşleştiğinde lazy-loaded edilir.
...
Bir AI geliştirme projesinde tek bir ProjectLens taraması, zincirdeki her halkayı su yüzüne çıkarır: ham not defterlerinden eğitime, modellemeye, embedding'lere, agentik orkestrasyona, UI bileşenlerine ve üretim deployment'larına — hepsi **tek bir kapsülde**, bütçe içinde.

---

## Oturum kancaları — 5 üretim kancası

5 kanca bir Claude Code oturumunda birleşir. Token tasarrufları oturumun tüm yaşam süresi boyunca üst üste birikir.
...
**Cowork sınırlaması:** Cowork'ün kanca yüzeyinde yalnızca SessionStart ateşlenir. Tarama motoru, kapsül oluşturma ve `/projectlens compact` çalışmaya devam eder — ancak kanca güdümlü 5 optimizasyon yalnızca Claude Code'un terminal CLI'sinde etkinleşir.

---

## Konuşma sıkıştırıcı

Uzun oturumlar bağlamı yer. Sıkıştırıcı saniyeler içinde 8–25k token geri kazanır.
...
Sıkıştırıcı, `activity_hook` tarafından kaydedilen oturum durumunu okur ve aşağıdakileri özetleyen bir `WORKING_CONTEXT.md` oluşturur:
...
Boş durum uyarısı: PostToolUse kancaları ateşlenmediyse (Cowork'te tipik), sıkıştırıcı işi yakaladığını iddia etmek yerine net bir tanı çıktısı verir.

---

## Token ekonomisi — somut sayılar

Tasarruflar nereden geliyor — üretim kullanımından somut sayılar:
...
**Maliyet örneği (Opus fiyatlandırma, $15/Mtok giriş):** önceden giriş tokenlerinde ~$3 maliyeti olan 4 saatlik bir kodlama oturumu, tüm kancalar aktifken ~$0,45–$0,90'a düşer.

---

## Testler ve performans bütçeleri

**527 birim testi** + **17 CI tarafından zorlanan performans ve güvenlik bütçesi** macOS / Linux / Windows × Python 3.9–3.12 üzerinde her commit'te çalışır.

Sentetik proje boyutlarında ölçülen performans:
...
Kapsül boyutu, **proje boyutundan bağımsız olarak katman bütçesi ile sınırlı** kalır — doğrusal tarama süresi, sabit çıktı. Mimari hendek budur.

CI'de zorlanan sıkı sınırlar:
...
Adaptör eklemek bunlardan hiçbirini geriletemez. Bir bütçeyi ihlal eden Pull Request'ler CI'yi başarısız yapar.
...
**`/projectlens compact --llm` komutunu açıkça çalıştırmadığınız sürece cihaz dışına hiçbir şey gönderilmez**. İstatistik ve bellek dosyaları düz JSON'dur — denetlenebilir, silinebilir, PII yok.
...
Tüm kalıcı yüzeylerin belgelenmiş env var opt-out'ları vardır:
...
Yeniden etkinleştirmek için unset yapın. Başka hiçbir şey değişmez — eklenti dosyaları taşınmaz, veri kaybolmaz.
...
## Genişlet — kendi adaptörünüzü yazın

Adaptör SDK'sı kasıtlı olarak küçüktür. Her adaptör ~80–120 LOC'dir.
...
Her adaptörün uyması gereken sözleşme:
...
`<your-project>/.projectlens/frameworks/` içine `.py` dosyaları bırakın. Opt-in yaptığınızda her tarama başına otomatik keşfedilirler:

```bash
export PROJECTLENS_USER_ADAPTERS=1
```

Güven kararı, taranan kodda değil shell'inizde gerçekleşir — sizi güvenilmeyen depodaki kötü niyetli adaptörlerden korur.

---

## Alternatiflerle karşılaştırma

ProjectLens, AI kodlama asistanı alanındaki **hafif, deterministik yönlendirme katmanıdır**. Semantik arama araçlarını yerine geçmek yerine tamamlar.
...
C: Temel tarama (dosya yürüyüşü, dil algılama, semboller) JS, TS, Go, Java, Rust, Ruby, PHP, C/C++ ve ~20 diğer dilde çalışır. Şu anda 30 framework'e özgü adaptör çoğunlukla Python odaklıdır, JS/web tarafını Vue SFC + Tailwind + Docker Compose karşılar. JS/TS framework adaptörleri (Next.js, Astro, SvelteKit, NestJS) yol haritasında.

**Q: Will it slow down my Claude Code session?**
C: Hook alt süreç başlatması soğuk 250 ms ile sınırlıdır (tipik olarak sıcak 20–30 ms). Hook çıktısı olay başına 500 token ile sınırlıdır. Her iki sınır da CI tarafından zorlanır. Taramanın kendisi prompt başına değil, talep üzerine çalışır.

**Q: Does it send my code anywhere?**
C: Hayır, `/projectlens compact --llm` komutunu açıkça çalıştırmadıkça VE `ANTHROPIC_API_KEY` ayarlı olmadıkça. O zaman bile yalnızca oturum aktivitesi özeti (dosya yolları, komut adları, test sonuçları) gönderilir — dosya içerikleri asla.

**Q: Is the capsule different from a README?**
C: README **insanlara** projenin ne yaptığını anlatır. Kapsül **ajanınıza** projenin neler içerdiğini anlatır: rotalar, modeller, eğitim döngüleri, vektör indeksleri, deploymentlar vb. Farklı kitle, farklı çıktı, her ikisinin de yeri var.
...
**Q: Is it free?**
C: Evet — MIT lisanslı. Abonelik yok, cihaz dışı telemetri yok, gerekli pip bağımlılığı yok.
...
GitHub issue'larıyla oy verin — ilk hangisi gelmeli?
...
⭐ ProjectLens size token kazandırıyorsa bu repo'ya yıldız verin. Kullandığınız bir framework kapsanmıyorsa bir issue açın — adaptörlerin çoğu ~100 LOC ve önceliklendireceğiz.
