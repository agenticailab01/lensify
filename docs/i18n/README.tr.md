...
15. [Genişlet — kendi adaptörünüzü yazın](#extending-lensify)
...
Lensify, bu yönlendirme aşamasını **tek bir tarama** (100 ms altı) ile değiştirir; bu tarama, token ile sınırlı, framework-bilinçli bir bağlam bloğu üretir. Ajan, düzinelerce dosya yerine **bir kapsül** okur. Token kullanımı, sadece yönlendirme tasarruflarından %70–90 düşer — ve 5 oturum kancası bunun üzerine ek tasarruflar ekler.

**Yaptığı ödün:** semantik vektör araması (daha yavaş, embedding maliyeti, jenerik) yerine deterministik yapısal çıkarma (hızlı, ücretsiz, framework-bilinçli). Lensify, Cursor'un `@codebase` ve Sourcegraph Cody gibi semantik araçların yanında çalışır — onlara karşı değil. Anlık yönlendirme için Lensify'i kullanın; ajan, yapı yerine anlam ile belirli bir şeyi bulması gerektiğinde semantik aramaya başvurun.
...
Lensify doğru derinliği otomatik seçer. Yalnızca güçlü bir nedeniniz olduğunda `--tier T1|T2|T3` ile geçersiz kılın.
...
Sohbette geçersiz kılma ipuçları — Lensify niyeti okur:
...
Bir AI geliştirme projesinde tek bir Lensify taraması, zincirdeki her halkayı su yüzüne çıkarır: ham not defterlerinden eğitime, modellemeye, embedding'lere, agentik orkestrasyona, UI bileşenlerine ve üretim deployment'larına — hepsi **tek bir kapsülde**, bütçe içinde.
...
**Cowork sınırlaması:** Cowork'ün kanca yüzeyinde yalnızca SessionStart ateşlenir. Tarama motoru, kapsül oluşturma ve `/lensify compact` çalışmaya devam eder — ancak kanca güdümlü 5 optimizasyon yalnızca Claude Code'un terminal CLI'sinde etkinleşir.
...
**`/lensify compact --llm` komutunu açıkça çalıştırmadığınız sürece cihaz dışına hiçbir şey gönderilmez**. İstatistik ve bellek dosyaları düz JSON'dur — denetlenebilir, silinebilir, PII yok.
...
`<your-project>/.lensify/frameworks/` içine `.py` dosyaları bırakın. Opt-in yaptığınızda her tarama başına otomatik keşfedilirler:
...
Lensify, AI kodlama asistanı alanındaki **hafif, deterministik yönlendirme katmanıdır**. Semantik arama araçlarını yerine geçmek yerine tamamlar.
...
C: Hayır, `/lensify compact --llm` komutunu açıkça çalıştırmadıkça VE `ANTHROPIC_API_KEY` ayarlı olmadıkça. O zaman bile yalnızca oturum aktivitesi özeti (dosya yolları, komut adları, test sonuçları) gönderilir — dosya içerikleri asla.
...
⭐ Lensify size token kazandırıyorsa bu repo'ya yıldız verin. Kullandığınız bir framework kapsanmıyorsa bir issue açın — adaptörlerin çoğu ~100 LOC ve önceliklendireceğiz.
