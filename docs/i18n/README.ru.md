...
15. [Расширение — напишите свой адаптер](#extending-lensify)
...
Lensify автоматически выбирает правильную глубину. Переопределяйте с `--tier T1|T2|T3` только когда есть веская причина.
...
Подсказки переопределения в чате — Lensify читает намерение:
...
Одно сканирование Lensify на проекте разработки AI выявляет каждое звено цепи: от сырых ноутбуков через обучение, моделирование, эмбеддинги, агентскую оркестрацию, UI-компоненты до продакшен-деплоев — всё в **одной капсуле**, в пределах бюджета.
...
**Ограничение Cowork:** только SessionStart срабатывает на поверхности хуков Cowork. Движок сканирования, генерация капсул и `/lensify compact` всё ещё работают — но 5 оптимизаций, управляемых хуками, активируются только в терминальном CLI Claude Code.
...
**Ничто не отправляется за пределы устройства**, если только вы явно не запустите `/lensify compact --llm`. Файлы статистики и памяти — простой JSON: проверяемый, удаляемый, без PII.
...
Поместите `.py` файлы в `<your-project>/.lensify/frameworks/`. После opt-in они авто-обнаруживаются при каждом сканировании:
...
Lensify — это **лёгкий, детерминированный слой ориентации** в пространстве ассистентов кодирования AI. Он дополняет инструменты семантического поиска, а не заменяет их.
...
О: Нет, если только вы не запустите явно `/lensify compact --llm` И не установите `ANTHROPIC_API_KEY`. Даже тогда отправляется только сводка активности сессии (пути файлов, имена команд, результаты тестов) — никогда содержимое файлов.
...
⭐ Поставьте звезду этому репо, если Lensify экономит вам токены. Откройте issue, если используемый вами фреймворк не покрыт — большинство адаптеров ~100 LOC, и мы приоритизируем.
