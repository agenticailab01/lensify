...
> 🌐 **Русский** — Назад к английскому: [English](../../README.md)
...
> **Адаптивная линза проекта одиночного сканирования + оптимизированная по токенам капсула контекста для AI-агентов программирования.** Сокращает токены ориентации на 70–90%. Распознавание фреймворков на протяжении всего жизненного цикла разработки AI. Чистая стандартная библиотека. Лицензия MIT.

**📖 Читайте на вашем языке:** 🇬🇧 [← Назад к английскому](../../README.md) · 🇨🇳 [简体中文](README.zh-CN.md) · 🇹🇼 [繁體中文](README.zh-TW.md) · 🇯🇵 [日本語](README.ja.md) · 🇰🇷 [한국어](README.ko.md) · 🇩🇪 [Deutsch](README.de.md) · 🇫🇷 [Français](README.fr.md) · 🇪🇸 [Español](README.es.md) · 🇮🇳 [हिन्दी](README.hi.md) · 🇧🇷 [Português](README.pt.md) · 🇸🇦 [العربية](README.ar.md) · 🇮🇹 [Italiano](README.it.md) · 🇵🇱 [Polski](README.pl.md) · 🇳🇱 [Nederlands](README.nl.md) · 🇹🇷 [Türkçe](README.tr.md) · 🇺🇦 [Українська](README.uk.md) · 🇻🇳 [Tiếng Việt](README.vi.md) · 🇮🇩 [Bahasa Indonesia](README.id.md) · 🇸🇪 [Svenska](README.sv.md) · 🇬🇷 [Ελληνικά](README.el.md) · 🇷🇴 [Română](README.ro.md) · 🇨🇿 [Čeština](README.cs.md) · 🇫🇮 [Suomi](README.fi.md) · 🇩🇰 [Dansk](README.da.md) · 🇳🇴 [Norsk](README.no.md) · 🇭🇺 [Magyar](README.hu.md) · 🇹🇭 [ภาษาไทย](README.th.md) · 🇺🇿 [O'zbekcha](README.uz.md)

---

## Содержание

1. [Почему ProjectLens](#why-projectlens)
2. [С первого взгляда](#at-a-glance)
3. [Быстрый старт](#quick-start)
4. [Как это работает](#how-it-works)
5. [Установка по инструменту (4 канала)](#installation-by-tool)
6. [Адаптивные уровни — T1 / T2 / T3](#adaptive-tiers)
7. [Покрытие фреймворков — 30 адаптеров в 8 пакетах](#framework-coverage)
8. [Хуки сессии — 5 продакшен-хуков](#session-hooks)
9. [Компактор разговоров](#conversation-compactor)
10. [Экономика токенов — конкретные числа](#token-economics)
11. [Тесты и бюджеты производительности](#tests--performance)
12. [Безопасность и управление](#security--governance)
13. [Переменные среды конфигурации](#configuration)
14. [Структура проекта](#project-structure)
15. [Расширение — напишите свой адаптер](#extending-projectlens)
16. [Сравнение с альтернативами](#comparison)
17. [FAQ](#faq)
18. [Дорожная карта](#roadmap)
19. [Вклад и лицензия](#contributing--license)

---

## Почему ProjectLens

У современных AI-агентов кодирования есть проблема контекстного окна: чем больше проект, тем больше токенов они сжигают просто **на ориентацию**. Типичный поток онбординга читает 20–40 файлов, прежде чем агент сможет делать полезную работу — это 10–30k токенов, потраченных на понимание, а не на решение реальной проблемы пользователя.

ProjectLens заменяет эту фазу ориентации **одиночным сканированием** (менее 100 мс), которое создает токен-ограниченный, фреймворк-осознанный контекстный блок. Агент читает **одну капсулу** вместо десятков файлов. Использование токенов падает на 70–90% только за счет экономии на ориентации — и 5 хуков сессии добавляют дополнительные сбережения сверху.

**Компромисс:** детерминированное структурное извлечение (быстрое, бесплатное, фреймворк-осознанное) вместо семантического векторного поиска (медленнее, стоимость эмбеддинга, общий). ProjectLens работает рядом с семантическими инструментами, такими как `@codebase` от Cursor и Sourcegraph Cody — не против них. Используйте ProjectLens для мгновенной ориентации; обращайтесь к семантическому поиску, когда агенту нужно найти что-то конкретное по смыслу, а не по структуре.
...
📖 Полные пошаговые инструкции в **[`USER-INSTALL.md`](../../USER-INSTALL.md)**
---

## Как это работает

Движок сканирования выполняет пять фаз за вызов:
...
Типичное время выполнения: **30 мс** на средних проектах, **113 мс** на проекте из 500 файлов. Сканирование никогда не перечитывает файлы между фазами.

**Два артефакта за сканирование:**
...
## Установка по инструменту (4 канала)

Четыре канала распространения используют один и тот же движок сканирования. Выберите тот, что подходит вашему инструменту.

### Channel 1 — Claude Code / Cowork plugin (recommended)

Полный опыт: все 5 хуков срабатывают, слэш-команды, статусная строка, загрузчик памяти.
...
Чистый stdlib JSON-RPC 2.0 stdio сервер. Шаг `pip install` для самого сервера не нужен — он запускается прямо из клонированного репо, используя только стандартную библиотеку Python.
...
## Адаптивные уровни — T1 / T2 / T3

ProjectLens автоматически выбирает правильную глубину. Переопределяйте с `--tier T1|T2|T3` только когда есть веская причина.
...
Подсказки переопределения в чате — ProjectLens читает намерение:
...
## Покрытие фреймворков — 30 адаптеров в 8 пакетах

30 адаптеров в 8 пакетах. Каждый адаптер занимает ~80–120 LOC, лениво загружается только когда его сигнатура фреймворка совпадает с проектом.
...
Одно сканирование ProjectLens на проекте разработки AI выявляет каждое звено цепи: от сырых ноутбуков через обучение, моделирование, эмбеддинги, агентскую оркестрацию, UI-компоненты до продакшен-деплоев — всё в **одной капсуле**, в пределах бюджета.

---

## Хуки сессии — 5 продакшен-хуков

5 хуков накапливаются в сессии Claude Code. Экономия токенов растёт на протяжении всего срока жизни сессии.
...
**Ограничение Cowork:** только SessionStart срабатывает на поверхности хуков Cowork. Движок сканирования, генерация капсул и `/projectlens compact` всё ещё работают — но 5 оптимизаций, управляемых хуками, активируются только в терминальном CLI Claude Code.

---

## Компактор разговоров

Длинные сессии съедают контекст. Компактор возвращает 8–25k токенов за секунды.
...
Компактор читает состояние сессии, записанное `activity_hook`, и генерирует `WORKING_CONTEXT.md`, суммирующий:
...
Предупреждение о пустом состоянии: если ни один PostToolUse хук не сработал (типично для Cowork), компактор выводит чёткий диагноз вместо того, чтобы притворяться, что захватил работу.

---

## Экономика токенов — конкретные числа

Откуда берутся сбережения — конкретные цифры из продакшен-использования:
...
**Пример стоимости (тариф Opus, $15/Mtok вход):** 4-часовая сессия кодирования, которая раньше стоила ~$3 во входных токенах, падает до ~$0.45–$0.90 при всех активных хуках.

---

## Тесты и бюджеты производительности

**527 модульных тестов** + **17 принудительных CI бюджетов производительности и безопасности** запускаются на каждом коммите на macOS / Linux / Windows × Python 3.9–3.12.

Измеренная производительность на синтетических размерах проектов:
...
Размер капсулы остаётся **ограниченным бюджетом уровня независимо от размера проекта** — линейное время сканирования, постоянный выход. Это архитектурный ров.

Жёсткие ограничения, принудительно применяемые в CI:
...
Добавление адаптеров не может регрессировать ни одно из них. Pull Requests, нарушающие бюджет, проваливают CI.
...
**Ничто не отправляется за пределы устройства**, если только вы явно не запустите `/projectlens compact --llm`. Файлы статистики и памяти — простой JSON: проверяемый, удаляемый, без PII.
...
Все постоянные поверхности имеют задокументированные env var опт-ауты:
...
Сбросьте, чтобы повторно включить. Больше ничего не меняется — никакие файлы плагина не перемещаются, никакие данные не теряются.
...
## Расширение — напишите свой адаптер

SDK адаптера намеренно мал. Каждый адаптер ~80–120 LOC.
...
Контракт, которому должен следовать каждый адаптер:
...
Поместите `.py` файлы в `<your-project>/.projectlens/frameworks/`. После opt-in они авто-обнаруживаются при каждом сканировании:

```bash
export PROJECTLENS_USER_ADAPTERS=1
```

Решение о доверии принимается в вашем shell, а не в сканируемом коде — защищая вас от вредоносных адаптеров в недоверенных репо.

---

## Сравнение с альтернативами

ProjectLens — это **лёгкий, детерминированный слой ориентации** в пространстве ассистентов кодирования AI. Он дополняет инструменты семантического поиска, а не заменяет их.
...
О: Базовое сканирование (обход файлов, определение языка, символы) работает на JS, TS, Go, Java, Rust, Ruby, PHP, C/C++ и ~20 других языках. 30 адаптеров, специфичных для фреймворков, в настоящее время в основном Python-ориентированы, при этом Vue SFC + Tailwind + Docker Compose покрывают сторону JS/web. Адаптеры фреймворков JS/TS (Next.js, Astro, SvelteKit, NestJS) — в дорожной карте.

**Q: Will it slow down my Claude Code session?**
О: Запуск подпроцесса хука ограничен 250 мс в холодном режиме (обычно 20–30 мс в горячем). Выход хука ограничен 500 токенами на событие. Оба предела принудительно применяются CI. Само сканирование выполняется по требованию, а не на каждый промпт.

**Q: Does it send my code anywhere?**
О: Нет, если только вы не запустите явно `/projectlens compact --llm` И не установите `ANTHROPIC_API_KEY`. Даже тогда отправляется только сводка активности сессии (пути файлов, имена команд, результаты тестов) — никогда содержимое файлов.

**Q: Is the capsule different from a README?**
О: README рассказывает **людям**, что делает проект. Капсула говорит вашему **агенту**, что содержит проект: маршруты, модели, обучающие циклы, векторные индексы, деплои и т.д. Разная аудитория, разный выход, оба имеют своё место.
...
**Q: Is it free?**
О: Да — лицензия MIT. Без подписки, без телеметрии за пределы устройства, без обязательных pip-зависимостей.
...
Голосуйте на GitHub issues — что должно быть первым?
...
⭐ Поставьте звезду этому репо, если ProjectLens экономит вам токены. Откройте issue, если используемый вами фреймворк не покрыт — большинство адаптеров ~100 LOC, и мы приоритизируем.
