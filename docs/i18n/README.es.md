...
> 🌐 **Español** — Volver al inglés: [English](../../README.md)
...
> **Lente de proyecto adaptativa de un solo escaneo + cápsula de contexto optimizada en tokens para agentes de codificación de IA.** Reduce los tokens de orientación en un 70–90 %. Consciente de los frameworks en todo el ciclo de vida del desarrollo de IA. Biblioteca estándar pura. Con licencia MIT.

**📖 Léelo en tu idioma:** 🇬🇧 [← Volver al inglés](../../README.md) · 🇨🇳 [简体中文](README.zh-CN.md) · 🇹🇼 [繁體中文](README.zh-TW.md) · 🇯🇵 [日本語](README.ja.md) · 🇰🇷 [한국어](README.ko.md) · 🇩🇪 [Deutsch](README.de.md) · 🇫🇷 [Français](README.fr.md) · 🇮🇳 [हिन्दी](README.hi.md) · 🇧🇷 [Português](README.pt.md) · 🇷🇺 [Русский](README.ru.md) · 🇸🇦 [العربية](README.ar.md) · 🇮🇹 [Italiano](README.it.md) · 🇵🇱 [Polski](README.pl.md) · 🇳🇱 [Nederlands](README.nl.md) · 🇹🇷 [Türkçe](README.tr.md) · 🇺🇦 [Українська](README.uk.md) · 🇻🇳 [Tiếng Việt](README.vi.md) · 🇮🇩 [Bahasa Indonesia](README.id.md) · 🇸🇪 [Svenska](README.sv.md) · 🇬🇷 [Ελληνικά](README.el.md) · 🇷🇴 [Română](README.ro.md) · 🇨🇿 [Čeština](README.cs.md) · 🇫🇮 [Suomi](README.fi.md) · 🇩🇰 [Dansk](README.da.md) · 🇳🇴 [Norsk](README.no.md) · 🇭🇺 [Magyar](README.hu.md) · 🇹🇭 [ภาษาไทย](README.th.md) · 🇺🇿 [O'zbekcha](README.uz.md)

---

## Tabla de contenidos

1. [Por qué ProjectLens](#why-projectlens)
2. [De un vistazo](#at-a-glance)
3. [Inicio rápido](#quick-start)
4. [Cómo funciona](#how-it-works)
5. [Instalación por herramienta (4 canales)](#installation-by-tool)
6. [Niveles adaptativos — T1 / T2 / T3](#adaptive-tiers)
7. [Cobertura de frameworks — 30 adaptadores en 8 paquetes](#framework-coverage)
8. [Hooks de sesión — 5 hooks de producción](#session-hooks)
9. [Compactador de conversación](#conversation-compactor)
10. [Economía de tokens — cifras concretas](#token-economics)
11. [Pruebas y presupuestos de rendimiento](#tests--performance)
12. [Seguridad y gobernanza](#security--governance)
13. [Variables de entorno de configuración](#configuration)
14. [Estructura del proyecto](#project-structure)
15. [Extiéndelo — escribe tu propio adaptador](#extending-projectlens)
16. [Comparación con alternativas](#comparison)
17. [Preguntas frecuentes](#faq)
18. [Hoja de ruta](#roadmap)
19. [Contribuir y licencia](#contributing--license)

---

## Por qué ProjectLens

Los agentes de codificación de IA modernos tienen un problema de ventana de contexto: cuanto más grande es el proyecto, más tokens queman solo en **orientarse**. Un flujo de incorporación típico lee 20-40 archivos antes de que el agente pueda hacer trabajo útil — eso son 10-30k tokens gastados en comprender, no en resolver el problema real del usuario.

ProjectLens reemplaza esa fase de orientación con un **único escaneo** (sub-100 ms) que produce un bloque de contexto limitado en tokens y consciente de frameworks. El agente lee **una cápsula** en lugar de docenas de archivos. El uso de tokens cae 70-90% solo por los ahorros de orientación — y los 5 hooks de sesión apilan ahorros adicionales encima.

**El trade-off:** extracción estructural determinista (rápida, gratuita, consciente de frameworks) en lugar de búsqueda vectorial semántica (más lenta, coste de embedding, genérica). ProjectLens trabaja junto a herramientas semánticas como `@codebase` de Cursor y Sourcegraph Cody — no en su contra. Usa ProjectLens para orientación instantánea; recurre a la búsqueda semántica cuando el agente necesite encontrar algo específico por significado en lugar de estructura.
...
📖 Instrucciones detalladas paso a paso en **[`USER-INSTALL.md`](../../USER-INSTALL.md)**
---

## Cómo funciona

El motor de escaneo ejecuta cinco fases por invocación:
...
Tiempo de ejecución típico: **30 ms** en proyectos medianos, **113 ms** en un proyecto de 500 archivos. El escaneo nunca vuelve a leer archivos entre fases.

**Dos artefactos por escaneo:**
...
## Instalación por herramienta (4 canales)

Cuatro canales de distribución comparten el mismo motor de escaneo. Elige el que coincida con tu herramienta.

### Channel 1 — Claude Code / Cowork plugin (recommended)

La experiencia completa: los 5 hooks se disparan, comandos slash, barra de estado, cargador de memoria.
...
Servidor JSON-RPC 2.0 stdio puro de stdlib. No se necesita paso `pip install` para el servidor en sí — se ejecuta directamente desde el repo clonado usando solo la biblioteca estándar de Python.
...
## Niveles adaptativos — T1 / T2 / T3

ProjectLens elige automáticamente la profundidad correcta. Anula con `--tier T1|T2|T3` solo cuando tengas una buena razón.
...
Pistas de anulación en el chat — ProjectLens lee la intención:
...
## Cobertura de frameworks — 30 adaptadores en 8 paquetes

30 adaptadores en 8 paquetes. Cada adaptador es ~80–120 LOC, cargado perezosamente solo cuando su firma de framework coincide con el proyecto.
...
Un solo escaneo de ProjectLens en un proyecto de desarrollo de IA hace aflorar cada eslabón de la cadena: desde notebooks crudos pasando por entrenamiento, modelado, embeddings, orquestación agéntica, componentes UI y despliegues de producción — todo en **una cápsula**, dentro del presupuesto.

---

## Hooks de sesión — 5 hooks de producción

5 hooks se componen a través de una sesión de Claude Code. Los ahorros de tokens se apilan durante toda la vida de la sesión.
...
**Limitación de Cowork:** solo SessionStart se dispara en la superficie de hooks de Cowork. El motor de escaneo, la generación de cápsula y `/projectlens compact` siguen funcionando — pero las 5 optimizaciones impulsadas por hooks solo se activan en el CLI de terminal de Claude Code.

---

## Compactador de conversación

Las sesiones largas consumen contexto. El compactador recupera 8-25k tokens en segundos.
...
El compactador lee el estado de sesión registrado por `activity_hook` y genera un `WORKING_CONTEXT.md` que resume:
...
Advertencia de estado vacío: si no se dispararon hooks PostToolUse (típico en Cowork), el compactador emite un diagnóstico claro en lugar de fingir que capturó trabajo.

---

## Economía de tokens — cifras concretas

De dónde provienen los ahorros — cifras concretas del uso en producción:
...
**Ejemplo de coste (precio de Opus, $15/Mtok de entrada):** una sesión de codificación de 4 horas que antes costaba ~$3 en tokens de entrada baja a ~$0,45–$0,90 con todos los hooks activos.

---

## Pruebas y presupuestos de rendimiento

**527 pruebas unitarias** + **17 presupuestos de rendimiento y seguridad impuestos por CI** se ejecutan en cada commit en macOS / Linux / Windows × Python 3.9–3.12.

Rendimiento medido en tamaños de proyectos sintéticos:
...
El tamaño de la cápsula permanece **limitado por el presupuesto del nivel independientemente del tamaño del proyecto** — tiempo de escaneo lineal, salida constante. Ese es el foso arquitectónico.

Topes duros impuestos en CI:
...
Añadir adaptadores no puede hacer retroceder ninguno de estos. Los Pull Requests que violan un presupuesto hacen fallar la CI.
...
**Nada se envía fuera del dispositivo** a menos que ejecutes explícitamente `/projectlens compact --llm`. Los archivos de estadísticas y memoria son JSON plano — auditables, eliminables, sin PII.
...
Todas las superficies persistentes tienen opt-outs documentados por variables de entorno:
...
Anula la configuración para reactivar. Nada más cambia — no se mueven archivos de plugin, no se pierden datos.
...
## Extiéndelo — escribe tu propio adaptador

El SDK de adaptador es intencionalmente pequeño. Cada adaptador es ~80–120 LOC.
...
El contrato que todo adaptador debe seguir:
...
Deja archivos `.py` en `<your-project>/.projectlens/frameworks/`. Se autodescubren por escaneo una vez que optas por participar:

```bash
export PROJECTLENS_USER_ADAPTERS=1
```

La decisión de confianza ocurre en tu shell, no en el código que se está escaneando — protegiéndote de adaptadores maliciosos en repos no confiables.

---

## Comparación con alternativas

ProjectLens es **la capa ligera y determinista de orientación** en el espacio de asistentes de codificación de IA. Complementa las herramientas de búsqueda semántica en lugar de reemplazarlas.
...
R: El escaneo base (recorrido de archivos, detección de lenguaje, símbolos) funciona en JS, TS, Go, Java, Rust, Ruby, PHP, C/C++ y ~20 otros lenguajes. Los 30 adaptadores específicos de framework son principalmente Python por ahora, con Vue SFC + Tailwind + Docker Compose cubriendo el lado JS/web. Los adaptadores de frameworks JS/TS (Next.js, Astro, SvelteKit, NestJS) están en la hoja de ruta.

**Q: Will it slow down my Claude Code session?**
R: El arranque de subproceso de hook está limitado a 250 ms en frío (típicamente 20–30 ms en caliente). La salida de hook está limitada a 500 tokens por evento. Ambos límites se imponen por CI. El escaneo en sí se ejecuta bajo demanda, no por prompt.

**Q: Does it send my code anywhere?**
R: No, a menos que ejecutes explícitamente `/projectlens compact --llm` Y tengas `ANTHROPIC_API_KEY` configurado. Incluso entonces, solo se envía el resumen de actividad de sesión (rutas de archivo, nombres de comandos, resultados de pruebas) — nunca el contenido de los archivos.

**Q: Is the capsule different from a README?**
R: Un README le dice a los **humanos** qué hace el proyecto. La cápsula le dice a tu **agente** qué contiene el proyecto: rutas, modelos, bucles de entrenamiento, índices vectoriales, despliegues, etc. Audiencia diferente, salida diferente, ambos tienen su lugar.
...
**Q: Is it free?**
R: Sí — licencia MIT. Sin suscripción, sin telemetría fuera del dispositivo, sin dependencias pip requeridas.
...
Vota con issues de GitHub — ¿qué debería venir primero?
...
⭐ Pon una estrella a este repo si ProjectLens te ahorra tokens. Abre un issue si un framework que usas no está cubierto — la mayoría de los adaptadores son ~100 LOC y los priorizaremos.
