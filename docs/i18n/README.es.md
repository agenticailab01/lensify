...
15. [Extiéndelo — escribe tu propio adaptador](#extending-lensify)
...
Lensify elige automáticamente la profundidad correcta. Anula con `--tier T1|T2|T3` solo cuando tengas una buena razón.
...
Pistas de anulación en el chat — Lensify lee la intención:
...
Un solo escaneo de Lensify en un proyecto de desarrollo de IA hace aflorar cada eslabón de la cadena: desde notebooks crudos pasando por entrenamiento, modelado, embeddings, orquestación agéntica, componentes UI y despliegues de producción — todo en **una cápsula**, dentro del presupuesto.
...
**Limitación de Cowork:** solo SessionStart se dispara en la superficie de hooks de Cowork. El motor de escaneo, la generación de cápsula y `/lensify compact` siguen funcionando — pero las 5 optimizaciones impulsadas por hooks solo se activan en el CLI de terminal de Claude Code.
...
**Nada se envía fuera del dispositivo** a menos que ejecutes explícitamente `/lensify compact --llm`. Los archivos de estadísticas y memoria son JSON plano — auditables, eliminables, sin PII.
...
Deja archivos `.py` en `<your-project>/.lensify/frameworks/`. Se autodescubren por escaneo una vez que optas por participar:
...
Lensify es **la capa ligera y determinista de orientación** en el espacio de asistentes de codificación de IA. Complementa las herramientas de búsqueda semántica en lugar de reemplazarlas.
...
R: No, a menos que ejecutes explícitamente `/lensify compact --llm` Y tengas `ANTHROPIC_API_KEY` configurado. Incluso entonces, solo se envía el resumen de actividad de sesión (rutas de archivo, nombres de comandos, resultados de pruebas) — nunca el contenido de los archivos.
...
⭐ Pon una estrella a este repo si Lensify te ahorra tokens. Abre un issue si un framework que usas no está cubierto — la mayoría de los adaptadores son ~100 LOC y los priorizaremos.
