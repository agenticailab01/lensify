...
15. [Utvid — skriv din egen adapter](#extending-lensify)
...
Lensify erstatter den orienteringsfasen med en **enkelt skanning** (under 100 ms) som produserer en tokenbegrenset, framework-bevisst kontekstblokk. Agenten leser **én kapsel** i stedet for dusinvis av filer. Tokenbruk faller 70-90% bare fra orienteringsbesparelser.

**Kompromisset:** deterministisk strukturell utvinning i stedet for semantisk vektorsøk. Lensify fungerer sammen med semantiske verktøy, ikke mot dem.
...
Lensify velger automatisk riktig dybde.
...
En enkelt Lensify-skanning på et AI-utviklingsprosjekt avslører hvert ledd i kjeden.
...
**Ingenting sendes off-device** med mindre du eksplisitt kjører `/lensify compact --llm`.
...
Lensify er **det lette, deterministiske orienteringslaget** i AI-kodingsassistentrommet.
...
S: Nei, med mindre du eksplisitt kjører `/lensify compact --llm` OG har `ANTHROPIC_API_KEY` satt.
...
⭐ Stjernemerk dette repoet hvis Lensify sparer deg tokens.
