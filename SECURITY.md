...
ProjectLens runs **locally inside Claude Code / Cowork** and operates on code the user already trusts enough to open in their editor. It performs:

- Read-only filesystem scans under a user-specified root
- A single optional outbound HTTPS call to `api.anthropic.com` (only when `/projectlens compact --llm` is invoked AND `ANTHROPIC_API_KEY` is set)
- Local-only `git` subprocess invocations (`git log`, `git ls-files`) for hotspot detection
- Local-only JSON file writes under `~/.projectlens/` (stats) and `<project>/.projectlens-memory/` (cross-session memory) — opt-out via env vars
...
| File writes | Out-of-bound writes | Only `<scan_root>/projectlens-out/`, `~/.projectlens/`, `<scan_root>/.projectlens-memory/` |
...
ProjectLens supports per-project adapter drop-ins at `<project>/.projectlens/frameworks/*.py`. These files are **imported as Python modules** — if a malicious repo ships an evil adapter, scanning that repo would execute that code inside the agent's environment.
...
|---|---|---|
| `POST https://api.anthropic.com/v1/messages` | Optional LLM-enhanced summary inside `/projectlens compact --llm` | User runs the slash command with `--llm` AND `ANTHROPIC_API_KEY` env var is set |
...
|---|---|---|---|
| Lifetime stats counters | `~/.projectlens/stats.json` | Permanent until you delete it | `PROJECTLENS_STATS=0` |
| Cross-session memory | `<project>/.projectlens-memory/*.json` | Per-project, capped at 50 entries (LRU) | `PROJECTLENS_MEMORY=0` |
| Session state | `<project>/projectlens-out/state.json` | Per-session only | `PROJECTLENS_DEDUP=0` (disables all hooks) |
| Capsule + lens artifacts | `<project>/projectlens-out/` | Per-project, regenerated each scan | n/a — created only on scan |

Nothing is sent off-device unless you explicitly run `/projectlens compact --llm`.
...
Email `agenticailab01@gmail.com` with subject `[ProjectLens-security]`. Please **do not file a public GitHub issue** for security reports.

Expected response time: 72 hours.

## Things ProjectLens is NOT
